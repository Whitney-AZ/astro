...
\tag{C16.01}\label{eq:c16-incoming-momenta}
$$

若一条腿代表出射粒子，传入顶点函数的便是该粒子动量的负值。这一约定使各条腿可以统一处理。在 $d=6-\varepsilon$ 时，先把有质量维数的耦合写成 $g_d=g\widetilde\mu^{\varepsilon/2}$；三价顶点的树级值为 $iZ_g g_d$。接下来的一阶修正来自三角图，圈动量按图C16a所示的方向选取。

::: {#c16-figure}

\begin{center}
\input{notes/figures/s16/triangle.tex}
\end{center}
...
写圈积分之前，先按第9章的方法确定收缩的组合系数。三阶展开的分母为 $3!(3!)^3$；将三个固定外标签分配给三个积分顶点有 $3!$ 种安排，在每个顶点选取与外场相接的场槽又有 $3^3$ 种。每个顶点余下的两个场槽须分别接向另外两个顶点，共有 $2^3$ 种选法；这些选择一经确定，全部内线的配对也就确定。因此净系数为

$$
\frac{3!\,3^3\,2^3}{3!(3!)^3}=1.
\tag{C16.02}\label{eq:c16-symmetry-factor}
$$
...
&\hspace{20mm}\times
 ...
 +O(g_d^5).
\end{aligned}
\tag{C16.03}\label{eq:c16-loop-amplitude}
$$
...
## 合并三个分母 {#c16-parameters}

利用[习题14.1已经证明的参数公式](#ex14-1)，可将三个分母合并，再通过平移圈动量完成平方。三个一次分母合并时的系数为 $\Gamma(3)=2!$，将它计入参数测度，定义
...
\end{aligned}
\tag{C16.04}\label{eq:c16-parameter-measure}
$$
...
\end{aligned}
\tag{C16.05}\label{eq:c16-combine-denominators}
$$
...
\tag{C16.06}\label{eq:c16-loop-shift}
$$
...
\tag{C16.07}\label{eq:c16-mass-first-form}
$$

合并分母与配方由此完成，平移的Jacobian为1。这些运算先在圈积分收敛、参数质量为正的区域进行，再沿共同的Feynman边界作解析延拓。所得常数项还可以利用动量守恒写得更对称：用式\eqref{eq:c16-incoming-momenta}消去内积后，两个平方项的系数分别化为 $x_1(1-x_1)-x_1x_2=x_1x_3$ 和 $x_2(1-x_2)-x_1x_2=x_2x_3$，于是得到对称形式

$$
D=m^2+x_1x_3k_1^2+x_2x_3k_2^2+x_1x_2k_3^2-i0.
\tag{C16.08}\label{eq:c16-symmetric-mass}
$$
...
完成平方后，圈积分只含一个二次式分母的幂，可以沿用第14章的Wick旋转。当 $D>0$ 时，取 $q^0=i\bar q_d$，于是 $d^dq=i\,d^d\bar q$，而 $q^2+D$ 变成 $\bar q^2+D$。旋转产生的 $i$ 与式\eqref{eq:c16-loop-amplitude}左侧的公共因子相消，留下带正号的欧氏积分：
...
\tag{C16.09}\label{eq:c16-euclidean-vertex}
$$

在计算前，先看积分在哪些维数收敛。大径向动量处，测度与分母合成 $\bar q^{d-1}\bar q^{-6}d\bar q=\bar q^{d-7}d\bar q$，因此紫外收敛要求 $d<6$；而 $D>0$ 排除了原点处由分母产生的红外发散。在附录E的径向公式中代入 $a=0,b=3$，角面积与径向积分相乘，得到
...
&=\frac{\Gamma(3-d/2)}
 {2(4\pi)^{d/2}}D^{\,d/2-3}.
\end{aligned}
\tag{C16.10}\label{eq:c16-radial-integral}
$$
...
\tag{C16.11}\label{eq:c16-dimensionally-regulated}
$$

为使顶点按六维量纲记述，定义 $V_3=\widetilde\mu^{-\varepsilon/2}V_{3,d}$，因而 $V_3/g=V_{3,d}/g_d$。下文使用这个无量纲比值，并在反项减除以后取有限的六维极限。为找出需要减去的部分，使用附录E推得的Gamma函数在零附近的展开，同时展开含尺度的幂：
...
\Gamma(\varepsilon/2)
 ...
 &=\frac2\varepsilon+
     ...
     +O(\varepsilon).
\end{aligned}
\tag{C16.12}\label{eq:c16-pole-expansion}
$$
...
=Z_g+\frac\alpha2
 ...
 +O(\alpha^2).
\tag{C16.13}\label{eq:c16-unsubtracted-log}
$$

式中已略去随调节量消失的 $O(\varepsilon)$ 项，剩下的极点须由顶点反项抵消。有限动量对数沿用同一 $D-i0$ 的分支，这一处方在继续到类时外动量时仍须保留。

## 怎样定义耦合常数 {#c16-coupling}

将顶点反项记为 $C$，即 $Z_g=1+C$。为了区分局部反项与动量依赖，把 $\ln(\mu^2/D)$ 拆成 $2\ln(\mu/m)-\ln(D/m^2)$，将式\eqref{eq:c16-unsubtracted-log}写为
...
+O(\alpha^2).
\tag{C16.14}\label{eq:c16-counterterm-isolation}
$$
...
C=-\alpha\left[\frac1\varepsilon+\ln\frac\mu m+\kappa_C\right]
  +O(\alpha^2)
\tag{C16.15}\label{eq:c16-c-counterterm}
$$
...
\tag{C16.16}\label{eq:c16-finite-vertex}
$$

辅助尺度 $\mu$ 已在固定的 $m,g,\kappa_C$ 下消去。还须确定的有限常数 $\kappa_C$，则关系到耦合的定义。传播子的质量和留数已经由粒子态的定义固定；对顶点，可以另选一个归一化条件来规定 $g$。用实验截面确定参数时，须采用该条件下的 $g$ 及相应的圈修正。改变归一化条件会改变参数 $g$ 的数值，只要完成参数换算，所描述的可测散射量仍应相同。

例如，用 $\kappa_C$ 与 $\kappa_C'$ 两种条件描述同一个顶点，并在当前阶数令 $g'=g+\delta g$。将两种参数写法代入式\eqref{eq:c16-finite-vertex}并比较，得到
...
\end{aligned}
\tag{C16.17}\label{eq:c16-finite-coupling-conversion}
$$

式中把 $\alpha'$ 换成 $\alpha$ 所产生的差已属于下一阶，因此一次有限的耦合换算便能补偿减除常数的改变。本章选择零动量处的顶点来定义耦合：所有外动量为零时，$D=m^2$，参数对数逐点为零，故

$$
\frac{V_3(0,0,0)}g=1-\alpha\kappa_C+O(\alpha^2).
\tag{C16.18}\label{eq:c16-zero-momentum-value}
$$
...
$$
V_3(0,0,0)=g.
\tag{C16.19}\label{eq:c16-zero-momentum-condition}
$$

由于非零质量粒子的三个外腿不能在零动量处同时在壳，这里规定的是离壳顶点的数值。按此定义的 $g$ 可以通过实际散射过程测定，计算散射量时沿用同一参数定义即可。这个条件也能逐阶延用：在更高阶将低阶已知项代入 $V_3(0,0,0)$，再调节 $C$ 的新一阶常数，就能继续维持零动量处的归一化。

## 一个可以算完的动量例 {#c16-soft-leg}

耦合确定以后，在当前阶数，顶点随外动量的变化由有限的参数积分给出。一般的三个不变量仍会留下二维积分；在式\eqref{eq:c16-parameter-measure}中取 $x_1=t,\ x_2=(1-t)v,\ x_3=(1-t)(1-v)$，Jacobian为 $1-t$，便可将三角形参数区改写成固定正方形：
...
\end{aligned}
\tag{C16.20}\label{eq:c16-square-parameter-integral}
$$

先在对数宗量为正的区域计算，再将其他动量取值沿规定的下岸延拓，这个表示便保留了顶点的完整参数依赖。为看清有限修正如何随动量改变，下面选一个可以直接积分的运动学例子。

取Euclidean动量 $k_1=p,\ k_2=-p,\ k_3=0$，记 $p^2=Q^2>0$、$u=Q^2/m^2$。一条外腿的动量为零后，参数质量只依赖一个参数：

$$
D=m^2+Q^2x_3(1-x_3),\qquad
J(u)=2\int_0^1dx\,(1-x)\ln[1+ux(1-x)].
\tag{C16.21}\label{eq:c16-one-soft-leg-parameter}
$$
...
\tag{C16.22}\label{eq:c16-symmetric-log-integral}
$$
...
\tag{C16.23}\label{eq:c16-even-log-integral}
$$
...
\end{aligned}
\tag{C16.24}\label{eq:c16-soft-leg-closed}
$$
...
+O(\alpha^2).
\tag{C16.25}\label{eq:c16-soft-leg-vertex}
$$

这个闭式同时包含小动量和大动量的行为。先看小 $u$ 区间，直接展开式\eqref{eq:c16-symmetric-log-integral}最为方便。由于 $0\le x(1-x)\le1/4$，在 $|u|<4$ 时对数级数一致收敛，可以逐项积分。前两项所需的积分为 $\int_0^1x(1-x)dx=1/6$ 和 $\int_0^1x^2(1-x)^2dx=1/30$，因而
...
&=1-\frac{\alpha u}{12}+\frac{\alpha u^2}{120}
 +O(\alpha u^3,\alpha^2).
\end{aligned}
\tag{C16.26}\label{eq:c16-soft-leg-low-momentum}
$$

当 $Q\to0$ 时，顶点回到定义所要求的 $g$；在有限但小的 $Q$ 处，圈图带来平滑的动量修正，其变化尺度由质量 $m$ 控制。

## 大动量为何产生对数 {#c16-high-energy}
...
\tag{C16.27}\label{eq:c16-soft-leg-large-log}
$$
...
&=1-\frac\alpha2
 ...
 +O(\alpha^2).
\end{aligned}
\tag{C16.28}\label{eq:c16-soft-leg-high-momentum}
$$

除了随动量增长的对数，闭式还给出了这个运动学下的有限常数。一般情形中，对数的系数可以直接从参数表示读出，但还须说明参数积分为何只留下有限的余项。先取Euclidean的非负不变量 $k_i^2=Q^2r_i$，保持 $r_i$ 固定且至少一个非零，定义
...
\end{aligned}
\tag{C16.29}\label{eq:c16-hard-ratios}
$$
...
\end{aligned}
\tag{C16.30}\label{eq:c16-endpoint-log-control}
$$

其中两个初等积分可由分部积分得到：$\int_0^1\ln x\,dx=-1$、$\int_0^1x\ln x\,dx=-1/4$，两项边界都因 $x^a\ln x\to0$ 而消失。由此可知 $|\ln x_1|+|\ln x_3|$ 可积；另一方面，$f$ 在紧参数区有上界，因而式\eqref{eq:c16-hard-ratios}中的对数上下界都受到可积函数的控制。取 $Q\to\infty$ 后，第二项趋于有限常数，最终得到
...
\end{aligned}
\tag{C16.31}\label{eq:c16-general-high-log}
$$

大对数正是这样从完整参数积分中产生的。其他允许的硬运动学可由同一解析函数继续得到；经过阈值时须保留对数的虚部，接近额外的软或共线边界时则须重新检查尺度比。在这些条件下，顶点的相对圈修正与第14章的自能相对修正一样，都会随能量按对数增长。一圈结果作为树级的小修正使用时，需要 $\alpha|\ln(Q^2/m^2)|\ll1$；超出这一范围后，可用[第28章的重整化群方法](#c28)重新组织这些大对数。
