...
第26章把未被分辨的粒子配置加在一起以后，截面中仍留下$\ln(s/m^2)$。当$s\gg m^2$时，这个大对数会使高阶圈修正变得重要。要理解它的来源，需要回到质量和耦合的定义：我们过去把$m$定义为传播子的极点质量，并在零动量处规定三点顶角等于$g$。这些条件将参数与低能物理直接联系起来，但用这组参数描述远高于质量的散射时，圈图中就会出现两个能标之比的对数。下面改变反项的有限部分，再通过同一物理量换算两组参数，看看怎样选择更适合高能计算的耦合定义。所需圈积分已在[第14章](#c14)和[第16章](#c16)求出，以下从这些积分的有限部分开始。

## 从极点条件到最小减除 {#c27-schemes}

仍取$d=6-\epsilon$，并用第14章的尺度$\mu^2=4\pi e^{-\gamma}\widetilde\mu^2$。为使两种方案的差别直接显现在反项中，先不指定它们的有限部分。置$z=k^2$、$a=x(1-x)$、$D=m^2+az-i0$，一圈自能写为
...
\end{aligned}
\tag{C27.01}\label{eq:c27-unsubtracted}
$$
...
\tag{C27.02}\label{eq:c27-unsubtracted-derivative}
$$
...
以前采用的极点方案（on-shell scheme，简称OS）要求$\Pi_{\rm OS}(-m^2)=\Pi'_{\rm OS}(-m^2)=0$，分别将参数质量定在物理极点上，并把留数归一为一。用这两个条件确定有限反项后，第14章的结果为
...
\tag{C27.03}\label{eq:c27-os-reminder}
$$
...
\tag{C27.04}\label{eq:c27-msbar-counterterms}
$$

这便定义了修正最小减除方案（modified minimal subtraction，$\overline{\mathrm{MS}}$）。这里的$C$用于三点顶角，稍后将与二点函数采用同一减除规定。结合上述尺度定义，式\eqref{eq:c27-msbar-counterterms}消去了维数积分中的极点，以及原始尺度下与极点相伴的$\ln4\pi-\gamma$。
...
\end{aligned}
\tag{C27.05}\label{eq:c27-ms-scale-conversion}
$$

第一行的差来自$\int D\,dx=m^2+z/6$，第二行则利用了归一化Feynman测度$\int dF_3=1$。改取$\nu=\widetilde\mu=\mu e^{-p/2}$后，尺度对数中的常数恰好补偿这两个差，恢复本书的有限核。下面统一使用$\overline{\mathrm{MS}}$和$\mu$，这一常数已经包含在尺度定义中。

将式\eqref{eq:c27-msbar-counterterms}代回自能，极点项相消，余下有限部分：

$$
\Pi(z)=-\frac{\alpha}{12}(z+6m^2)
+\frac{\alpha}{2}\int_0^1dx\,D\ln\frac D{\mu^2}
+O(\alpha^2).
\tag{C27.06}\label{eq:c27-msbar-self-energy}
$$
...
\end{aligned}
\tag{C27.07}\label{eq:c27-massless-offshell}
$$

虽然$z\ln z\to0$，导数仍在$z=0$发散。新方案因此给出了有限的离壳归一化，但要将它用于物理截面，仍需结合前章对共线粒子配置的处理。下面先保持$m>0$，利用孤立极点确定LSZ的外腿因子，再作高能展开。

## 物理质量和外腿留数 {#c27-pole-lsz}
...
\tag{C27.08}\label{eq:c27-pole-definition}
$$
...
$$
R^{-1}=1-\Pi'(-M^2).
\tag{C27.09}\label{eq:c27-residue-definition}
$$

具体地说，展开分母时先用极点方程消去常数项，余下$(z+M^2)[1-\Pi'(-M^2)]$，所以逆留数中的减号随之确定。这里取的是关于$k^2$的留数；若把传播子看成$k^0$的函数，变量代换还会带来$2\sqrt{\mathbf k^2+M^2}$的Jacobian。
...
\tag{C27.10}\label{eq:c27-lsz-one-leg}
$$
...
=\Pi(-m^2)+O(\alpha^2m^2).
\tag{C27.11}\label{eq:c27-pole-order}
$$

这一截断按$\alpha|\ln(\mu^2/m^2)|\ll1$使用；对数很大时，需要后面给出的跑动解重新组织展开。令$L_\mu=\ln(\mu^2/m^2)$。在$z=-m^2$处，$D_0=m^2(1-a)>0$，因而这里需要的都是阈值下的实积分。[式C14.40–44](#c14-finite-constants)已经给出
...
\tag{C27.13}\label{eq:c27-pole-integral}
$$

将这一积分乘上自能中的系数，再与式\eqref{eq:c27-msbar-self-energy}中的$-5\alpha m^2/12$相加，极点方程化为
...
\end{gathered}
\tag{C27.14}\label{eq:c27-physical-mass}
$$

常数$c'$由这些有限积分完全确定；给定减除方案后，它已经没有另行选择的自由。

留数可按同样的步骤求出。在式\eqref{eq:c27-unsubtracted-derivative}中代入$A$后，积分中$+1$所给的贡献与显式$-\alpha/12$相消，剩下的对数积分直接由前面的加权结果计算：
...
\end{aligned}
\tag{C27.15}\label{eq:c27-residue-evaluated}
$$

这里用$-m^2$代替$-M^2$，所引入的误差仍在二圈阶。两个有限常数$c'$和$c''$分别来自质量条件与留数条件。

## 质量为什么随尺度变化 {#c27-mass-running}

现在有了参数质量与物理质量的明确关系。物理质量$M$应当与任意选择的$\mu$无关，因此参数本身的变化必须补偿圈图中显式的尺度变化。对式\eqref{eq:c27-physical-mass}取平方根再取对数，将这一条件写成便于求导的形式：

$$
\ln M=\ln m+\frac{5\alpha}{12}
\left(\ln\frac\mu m+\frac{c'}2\right)+O(\alpha^2).
\tag{C27.16}\label{eq:c27-log-mass}
$$
...
\tag{C27.17}\label{eq:c27-rg-definitions}
$$
...
\tag{C27.18}\label{eq:c27-mass-chain-rule}
$$
...
\tag{C27.19}\label{eq:c27-mass-anomalous-dimension}
$$

增大$\mu$时，$m(\mu)$随之下降，正好补偿圈图中对数的变化，使$M$保持不变。这种由量子修正引起的尺度变化称为“反常量纲”（anomalous dimension）；质量原有的质量维数1仍由单位制确定。

## 三点顶角和方案之间的有限换算 {#c27-vertex-matching}

质量和场的归一化确定以后，还须用同一方案定义耦合。第16章三角图的归一化测度为$dF_3=2\,dx\,dy\,dw\,\delta(x+y+w-1)$，积分区域是$x,y,w\ge0$。取$D_3=m^2+xyk_1^2+ywk_2^2+wxk_3^2-i0$，并减去$C=-\alpha/\epsilon$，得到有限顶角及其零动量值
...
\tag{C27.20}\label{eq:c27-msbar-vertex}
$$

三角图的对称因子和参数积分沿用[第16章](#c16)的推导，变化只在有限减除。零动量时$D_3=m^2$，对数不再依赖参数，因而第二个结果由$\int dF_3=1$直接得到。
...
\end{aligned}
\tag{C27.21}\label{eq:c27-finite-coupling-conversion}
$$
...
\tag{C27.22}\label{eq:c27-finite-self-energy-conversion}
$$

右边只需使用共同的领先$m,g$，其差异乘上一圈项后属于更高阶；完整OS参数则是$M,g_{\rm OS}$。式中的第二项将极点放在物理质量处，第三项把留数归一为一。代入对数积分，便恢复式\eqref{eq:c27-os-reminder}，从而看出两种二点核怎样通过有限减除相互转换。
...
\tag{C27.23}\label{eq:c27-bare-parameters}
$$
...
\end{aligned}
\tag{C27.24}\label{eq:c27-finite-counterterms}
$$

例如，质量条件$0=\Pi(-m^2)+m^2(\delta A-\delta B)$与已求出的场反项差一起给出第二行。裸耦合不变则要求$g_{\rm OS}/g=1-\delta C+\tfrac32\delta A$，代入后得到与式\eqref{eq:c27-finite-coupling-conversion}相同的换算。场和参数必须一起改变，才能使两种图展开对应同一个散射过程。

## 重新计算高能散射 {#c27-inclusive}

现在用新参数重做第26章的高能散射，取$s\gg M^2$，并保持$t/s,u/s$远离零。树振幅仍由三条交换道相加得到：

$$
\mathcal T_0=-g^2\left(\frac1s+\frac1t+\frac1u\right).
\tag{C27.25}\label{eq:c27-hard-tree}
$$

要找出各通道的共同对数，先看任意一条交换道。令交换动量平方为$z$，自能的大$|z|$部分可直接使用式\eqref{eq:c27-massless-offshell}。对于该道的三点顶角，两个外宗量在领先质量阶为零，因此$D_3\simeq xyz$，其中$x,y$是Feynman参数。将尺度对数从参数积分中提出后，余下的对数积分可按单纯形几何计算：固定$x$时，剩余积分区间的长度为$1-x$，于是
...
\qquad
\int dF_3\ln(xy)=-3.
\tag{C27.26}\label{eq:c27-hard-triangle-integral}
$$
...
\tag{C27.27}\label{eq:c27-hard-subgraphs}
$$
...
\tag{C27.28}\label{eq:c27-hard-eleven}
$$

共同系数$11/12$的大小由两次顶角的$-1/2$与自能的$+1/12$相加确定，整体符号为负。对于$s$道，$\ell_{-s}=\ln(s/\mu^2)-i\pi$；$t,u$道分别多出$\ln(-t/s)$、$\ln(-u/s)$。固定散射角时，这些差保持有限，可以与共同的硬尺度对数分开。第20章已经求出的三个盒图是紫外有限的，领先部分只依赖角度比值及其对数，因而不再贡献$\ln\mu$。三道相加并乘上外腿因子后，振幅为

$$
\mathcal T=R^2\mathcal T_0
\left[1-\frac{11\alpha}{12}\ln\frac s{\mu^2}
+\alpha F_{\rm hard}(t/s,u/s)+O(\alpha^2)\right].
\tag{C27.29}\label{eq:c27-hard-amplitude}
$$

这里$F_{\rm hard}$收集了式\eqref{eq:c27-hard-eleven}中的常数、三道不同的有限对数以及[第20章盒积分](#c20-box)。它可以有虚部，但不依赖$\mu$，也不含发散的质量对数；需要完整有限角函数时，可直接代入这些已求出的表达式。本章关注的是各道共有的尺度对数。以上各式略去了随$m^2/s$及其对数趋零的质量修正，并在$\alpha|\ln(s/\mu^2)|$和$\alpha|\ln(s/m^2)|$尚小时按固定阶展开。

还有一份质量对数来自外态的归一化。由式\eqref{eq:c27-residue-evaluated}，$R^2=1-\alpha(L_\mu+c'')/6+O(\alpha^2)$，将它乘入振幅并保留同一阶，得到
...
\end{aligned}
\tag{C27.32}\label{eq:c27-log-cancellation}
$$
...
\tag{C27.33}\label{eq:c27-inclusive-rate}
$$

其中的有限函数为$F_{\rm obs}=2\operatorname{Re}F_{\rm hard}
-c''/3+(c-\ln4)/3$，在本章近似精度内不依赖$m,\mu$。这一相消使用了四条腿的共同权重；若固定两粒子纯初态，自动求和的只有两条出腿，式\eqref{eq:c27-real-splitting}的系数便应减半，仍留下入射共线质量对数。因此，式\eqref{eq:c27-inclusive-rate}的无质量极限与第26章的初态制备规定相联系。
...
\tag{C27.34}\label{eq:c27-rate-matching}
$$

将它乘上前章的$1-\tfrac32\alpha\ln(s/m^2)+\cdots$，两处质量对数合为$-\tfrac32\alpha\ln(s/\mu^2)$，有限项也随$c''$一起换算。这就解释了为何改变耦合定义能够改写前章留下的质量对数：描述同一过程的两种结果经过参数换算仍然相同。

## 从尺度独立性求出耦合的跑动 {#c27-beta}

新的表达式中出现了任意选定的$\mu$，物理截面却应与这个选择无关。既然式\eqref{eq:c27-inclusive-rate}显含$\mu$，耦合$g$就必须随尺度变化，使显式和隐式的依赖相互抵消。为将这一条件写成无量纲的对数关系，先把$|\mathcal T|_{\rm obs}^2$除以固定的运动学因子，使树级结果为$\alpha^2$，记所得量为$\mathcal R$。固定$s,t,u,\delta$后，式\eqref{eq:c27-inclusive-rate}可写成

$$
\ln\mathcal R
=2\ln\alpha+3\alpha\left[\ln\frac{\mu}{\mu_*}+C_2\right]
+O(\alpha^2).
\tag{C27.35}\label{eq:c27-log-observable}
$$
...
\tag{C27.36}\label{eq:c27-beta-chain-rule}
$$
...
\tag{C27.37}\label{eq:c27-beta-one-loop}
$$

这同时给出了前面求质量反常量纲时暂用的阶数。所得$\beta$在此阶不依赖探测角，因为$\delta$只进入有限常数，求尺度导数时不参与领先相消。

裸参数提供了同一尺度关系的另一种写法。将式\eqref{eq:c27-msbar-counterterms}代入裸质量和裸耦合，可得
...
\tag{C27.38}\label{eq:c27-bare-poles}
$$
...
\tag{C27.39}\label{eq:c27-bare-beta-check}
$$

设$\beta_d=-\epsilon\alpha+b\alpha^2+\cdots$，方程中有限的$O(\alpha)$部分为$b\alpha/2+3\alpha/4$，于是$b=-3/2$。同样固定$m_B^2$，便有$0=2\gamma_m-5\beta_d/(6\epsilon)+\cdots$，在此阶得到$\gamma_m=-5\alpha/12$。这两次求导都需要先保留$\epsilon$项，让它与极点相乘后取有限部分，最后才令$\epsilon$趋于零。更高阶时各极点怎样配合，将在第28章继续讨论。

## 选择能标与渐近自由 {#c27-flow}

运行方程使我们能够在不同能标之间换算耦合。只保留式\eqref{eq:c27-beta-one-loop}的首项，将未知量改为耦合的倒数，方程便化成

$$
\frac{d}{d\ln\mu}\frac1\alpha=\frac32.
\tag{C27.40}\label{eq:c27-inverse-coupling}
$$
...
\tag{C27.41}\label{eq:c27-running-coupling}
$$

这是截断微分方程的完整解，将由同一领先系数产生的一串尺度对数求和起来。对于正耦合，增大$\mu$会减小$\alpha$，这种行为称为渐近自由（asymptotic freedom）。因而在散射能量$\sqrt s$处取$\mu\simeq\sqrt s$，既可减小式\eqref{eq:c27-inclusive-rate}中的硬尺度对数，又能用较小的耦合组织高能展开。若$\delta$极小，角分辨率对数仍会很大，需要另行处理；这里的跑动求和针对共同的硬尺度部分。

沿相反方向降低$\mu$，耦合会逐渐增大。式\eqref{eq:c27-running-coupling}的分母在下面这个形式尺度处降为零：

$$
\Lambda_{\rm formal}
=\mu_1\exp\left[-\frac{2}{3\alpha(\mu_1)}\right]
\tag{C27.42}\label{eq:c27-formal-strong-scale}
$$

不过，在接近这一尺度以前，$\alpha$已不再小，截断$\beta$方程的微扰依据随之失效。因此这里得到的是弱耦合展开可能失效的尺度量级，形式分母的零点本身还不能确定可观测量的奇点。
...
本例的低能讨论还须结合第5章提出的真空问题。实$\varphi^3$势及其导数为

$$
V(\varphi)=\frac12m^2\varphi^2-\frac g6\varphi^3,\qquad
V'(\varphi)=\varphi\left(m^2-\frac g2\varphi\right).
\tag{C27.43}\label{eq:c27-cubic-vacuum}
$$

当$m^2>0$时，$\varphi=0$是局部极小值，另一驻点$\varphi_b=2m^2/g$满足$V''(\varphi_b)=-m^2$及$V(\varphi_b)=2m^6/(3g^2)$，因此形成一个局部势垒。越过势垒，沿$g\varphi>0$方向势能向下无界；当$m=0$时，连这个局部势垒也消失了。本章对有质量情形所作的计算，采用的正是围绕局部真空的微扰展开。这里的真空判断来自势的形状，而负$\beta$描述的是弱耦合下的尺度变化，两种性质分别由各自的计算得到。

量子色动力学提供了另一种情形：它在高能处耦合变弱，低能激发却要用无色强子描述。第73、82章将讨论这一问题；强耦合区适合使用的自由度可能与拉格朗日量中的微扰自由度很不相同。若某理论在小正耦合处$\beta>0$，降低能标就使耦合减小，这称为红外自由（infrared freedom）；沿相反方向提高能量，耦合则逐渐增大。量子电动力学的这类跑动将在第66章具体计算。最后，$\beta$还可能在非零耦合处出现零点，下一章将讨论这样的固定点。质量怎样与耦合同步变化，则由[习题27.1](#ex27-1)中的两条方程直接积分求出。
