...
第46章把自旋求和写成了矩阵迹，第47章又给出了求迹的规则。现在将这两步接起来，先计算电子与正电子的弹性散射，再计算电子与标量的弹性散射，最后把结果换成可以观察的角分布。相互作用仍取$g\varphi\bar\Psi\Psi$，因而这里的电子正电子过程由标量交换产生。以下保留树级振幅所给的$g^4$阶模平方，并对独立、未极化的入射束取平均。

## 电子正电子散射的四个迹 {#c48-pair-start}

将电子的入射、出射动量记为$p_1,p_1'$，正电子的记为$p_2,p_2'$。四条外腿满足$p_i^2=p_i'^2=-m^2$，动量守恒为$p_1+p_2=p_1'+p_2'$。Mandelstam变量定义为
...
\end{aligned}
\tag{C48.01}\label{eq:c48-pair-mandelstam}
$$
...
\end{aligned}
\tag{C48.02}\label{eq:c48-pair-dot-products}
$$

先列出这组运动学关系，求迹以后便可随时将外动量的内积换成不变量。接下来把固定自旋外积简记为$R_i=u_i\bar u_i$、$V_i=v_i\bar v_i$，出射量加prime。用四个系数分别收集两个图的平方及其干涉：
...
\end{aligned}
\tag{C48.03}\label{eq:c48-fixed-phi}
$$

前两项对应各图的平方，后两项保留两种干涉的矩阵次序。这些次序已在[第46章](#c46-two-lines)通过外旋量的指标收缩得到；现在补上各道分母，便有
...
\bigg].
\end{aligned}
\tag{C48.04}\label{eq:c48-pair-fixed-square}
$$

其中$M$是内部标量的质量。干涉前的相对负号继承自两个树图的费米置换号，第45章固定Fock次序所带的公共负号则已在模平方中消去。以下采用实耦合，并在离开内线极点的树级边界值上计算。
...
\tag{C48.05}\label{eq:c48-pair-spin-average}
$$

按照第46章的办法，可以先对各条外腿使用完整性关系。记

$$
U_i:=\sum_{\sigma_i}R_i=-\slashed p_i+m,\qquad
W_i:=\sum_{\sigma_i}V_i=-\slashed p_i-m ,
\tag{C48.06}\label{eq:c48-completeness}
$$

出射外腿也有同样的关系。在式\eqref{eq:c48-fixed-phi}的每一项中，四个自旋标签各出现一次，因此求和可以分别作用于四个外积，而保留它们原有的乘法次序。于是
...
\end{aligned}
\tag{C48.07}\label{eq:c48-pair-average-traces}
$$

自旋标签与外旋量至此都被消去，余下的是四条由动量和质量构成的矩阵链。

## 两个平方项 {#c48-pair-squares}
...
=2s-8m^2 .
\end{aligned}
\tag{C48.08}\label{eq:c48-annihilation-trace}
$$

最后一步使用了式\eqref{eq:c48-pair-dot-products}的内积关系。另一条湮灭链中的内积$p_1'p_2'$与此相同，所以两个迹具有相同的值。直接图的两个外积则带同号质量项，展开时常数项的符号随之改变，得到
...
\end{aligned}
\tag{C48.09}\label{eq:c48-other-pair-traces}
$$
...
\tag{C48.10}\label{eq:c48-pair-squares}
$$
...
## 干涉迹的六个质量项 {#c48-pair-interference}
...
\big]+m^4 .
\end{aligned}
\tag{C48.12}\label{eq:c48-interference-dots}
$$
...
&-r(s+t-u-2r)+r^2 .
\end{aligned}
\tag{C48.13}\label{eq:c48-interference-polynomial}
$$
...
=r^2-\frac{st}{2}.
\end{aligned}
\tag{C48.14}\label{eq:c48-three-square-reduction}
$$
...
\tag{C48.15}\label{eq:c48-pair-interference}
$$

第二种干涉也可由$p_2\leftrightarrow-p_1'$的交叉替换求出：四因子迹变为另一种次序，所产生的两个负号抵消，而最终多项式在$s\leftrightarrow t$下不变。因此两个平均干涉系数相等。第46章已由Dirac共轭说明，固定自旋的两种干涉迹互为复共轭；这里经过自旋求和，内积表达式又全为实数，便进一步得到相同的数值。
...
\bigg].
\end{aligned}
\tag{C48.16}\label{eq:c48-pair-result}
$$

中间一项的分子来自式\eqref{eq:c48-pair-fixed-square}中的负号乘$2\langle\Phi_{st}\rangle$。各分子与分母的质量维数均为4，模平方因而无量纲。固定$M>0$、$M^2\ne4m^2$，再取$s\to4m^2$、$t,u\to0$的弹性阈值极限，湮灭项和干涉项消失，只剩$16g^4m^4/M^4$，正好接回第46章用静止旋量直接求得的结果。

## 从平均模平方到截面 {#c48-cross-section}

要得到截面，还需加入[第11章](#c11)的相空间与通量因子。相空间、态归一和入射通量的处理与自旋无关，因此将其中固定自旋的模平方换成刚求得的平均值即可。沿用该章已经求出的二体相空间，写成
...
\end{aligned}
\tag{C48.17}\label{eq:c48-invariant-cross-section}
$$

此处$d\Phi_2$就是第11章的$d{\rm LIPS}_2$。在质心系令$q=|\mathbf p_1|$、$q'=|\mathbf p_1'|$，弹性运动学给出$q'=q=\sqrt{s-4m^2}/2$。在这些条件下，式\eqref{eq:c11-invariant-flux}和
\eqref{eq:c11-two-body-evaluated}分别成为
...
\qquad s>4m^2 .
\tag{C48.18}\label{eq:c48-pair-angular}
$$
...
\tag{C48.19}\label{eq:c48-pair-angle-substitution}
$$
...
{16\pi s(s-4m^2)} .
\tag{C48.20}\label{eq:c48-pair-t-cross-section}
$$

式\eqref{eq:c48-invariant-cross-section}同样适用于其它参考系，角变量的变换则要使用该参考系的Jacobian。若$M>2m$且$s$接近$M^2$，内部标量能够衰变，此时树式的极点须按[第25章](#c25)的办法处理，不能直接在共振区域积分固定阶结果。

还可以从这个截面看出一个有用的高能极限。作为补充，取$s\gg m^2,M^2$并保持散射角非零，使$|t|$与$s$同阶。式\eqref{eq:c48-pair-result}中三个分式的领先项各为1，因而
...
\end{aligned}
\tag{C48.22}\label{eq:c48-scalar-mandelstam}
$$
...
\end{aligned}
\tag{C48.23}\label{eq:c48-scalar-dot-products}
$$

第46章已经利用外腿Dirac方程，将该过程的振幅整理为$\bar u'Au$。现在代入电子自旋的完整性关系，得到
...
\end{aligned}
\tag{C48.24}\label{eq:c48-scalar-matrix}
$$

这里仅平均一个入射电子的自旋，所以系数为$1/2$。按迹的循环性，也可写成$\operatorname{tr}(APAP')$，结果与上式一致。外标量仍取第45章的稳定质量区间$0<M<2m$；在此范围内，两个内部费米分母不落在极点上，实耦合$g$给出$\bar A=A$。
...
\end{aligned}
\tag{C48.25}\label{eq:c48-scalar-four-traces}
$$
...
\bigg].
\end{aligned}
\tag{C48.26}\label{eq:c48-scalar-result-structure}
$$
...
\tag{C48.27}\label{eq:c48-h-definition}
$$
...
\end{aligned}
\tag{C48.28}\label{eq:c48-h-expansion}
$$
...
h:=(pp')&=\frac t2-r .
\end{aligned}
\tag{C48.30}\label{eq:c48-scalar-dot-abbreviations}
$$
...
\end{aligned}
\tag{C48.31}\label{eq:c48-ss-before-collection}
$$
...
&=7r^2-8rR+R^2 .
\end{aligned}
\tag{C48.32}\label{eq:c48-ss-coefficients}
$$
...
\tag{C48.33}\label{eq:c48-ss-polynomial}
$$
...
\tag{C48.34}\label{eq:c48-uu-polynomial}
$$

在原迹中，这一步交换的是两个标量动量，费米外腿和自旋平均均保持原样，因此没有新增的费米负号或平均系数。

## 两个干涉分子的系数 {#c48-scalar-interference}

接着取$x=-k,y=k'$。内积表给出$(p'k')=(pk)=a$和$(pk')=(p'k)=b$，因而式\eqref{eq:c48-h-evaluated}中的$(p+p')\cdot(-k+k')=-(a+b)+(b+a)=0$，全部线性于$x+y$的项相消。再记$\ell=(kk')=t/2-R$，余下各项为
...
\end{aligned}
\tag{C48.35}\label{eq:c48-su-before-collection}
$$
...
&-(R+4r)t+16r^2 .
\end{aligned}
\tag{C48.36}\label{eq:c48-su-quadratic}
$$
...
\end{aligned}
\tag{C48.37}\label{eq:c48-su-coefficients}
$$
...
\end{aligned}
\tag{C48.38}\label{eq:c48-su-polynomial}
$$

混合系数中最后的$M^4$取负号，和两个平方系数中的正号不同。将式\eqref{eq:c48-ss-polynomial}、
\eqref{eq:c48-uu-polynomial}和
\eqref{eq:c48-su-polynomial}代入
式\eqref{eq:c48-scalar-result-structure}，四个分子便全部化成了不变量。两个干涉分子虽相等，却不必为正；可观察的非负模平方由三组分式合在一起给出。

## 电子标量截面与两个极限 {#c48-scalar-checks}

换算截面时，入射通量要使用两种质量。质心系三动量满足$q^2=\lambda(s,m^2,M^2)/(4s)$，其中$\lambda(s,r,R)=(s-r-R)^2-4rR$；由于过程仍为弹性散射，$q'=q$。代入第11章的相空间与通量关系，就得到
...
u=2m^2+2M^2-s-t .
\end{aligned}
\tag{C48.39}\label{eq:c48-scalar-cross-section}
$$
...
&=4m^2(4m^2-M^2).
\end{aligned}
\tag{C48.40}\label{eq:c48-threshold-numerators}
$$

结合$m^2-s=-M(2m+M)$和$m^2-u=M(2m-M)$，两个平方分式各给$4m^2/M^2$，两个干涉分式合起来给$-8m^2/M^2$。三者恰好抵消，因而$\langle|\mathcal T|^2\rangle\to0$；这与第46章的静止旋量结果一致，也显出两道干涉在阈值处的作用。这里所取的$0<M<2m$保证两个分母均非零。

再看一个补充的固定角高能极限。令$s\gg m^2,M^2$，同时保持$-u/s$为非零的有限数。四个分子的领先项依次为$-su,-su,su,su$，代回各道分母，得到
...
\end{aligned}
\tag{C48.41}\label{eq:c48-scalar-high-energy}
$$

趋于后向$\theta\to\pi$时，$u$不再与$s$同阶，必须回到带质量的完整分母，因此不能将固定角展开直接用于该端点的积分。在其适用角区间内，电子标量散射的领先项仍有明显的角度依赖，与前一个例子不同。

式\eqref{eq:c48-ss-polynomial}至
\eqref{eq:c48-su-polynomial}给出了这个过程的全部四个分子。[习题48.2](../exercises/48.md#x48-2)将它们用于交叉过程：除了外旋量完整性核所给的负号，还需乘$1/2$，因为交叉前后初态自旋平均的数目不同。题解将从两边的迹式求出这个权重，并与相同末态的相空间除重区分开来。
