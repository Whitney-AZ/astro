...
T_0=\frac{\hbar}{E_0}.
\tag{C12.01}\label{eq:c12-natural-scales}
$$
...
\qquad [A]=a-b-r.
\tag{C12.02}\label{eq:c12-unit-restoration}
$$
...
方括号记录的是幂指数，所以乘积满足 $[AB]=[A]+[B]$，商则对应指数相减。按本书 $x^0=ct$ 的坐标约定，所有 $x^\mu$ 都是长度，因而有

$$
[m]=1,\qquad [x^\mu]=-1,\qquad
[\partial_\mu]=1,\qquad [d^dx]=-d.
\tag{C12.03}\label{eq:c12-basic-dimensions}
$$
...
\tag{C12.04}\label{eq:c12-polynomial-lagrangian}
$$
...
\tag{C12.05}\label{eq:c12-action-source}
$$

其中 $\mathcal N$ 沿第6章取为使零源泛函归一的常数。要确定各项的单位，只须考察指数：恢复普通单位后，指数应写成 $iS_{\rm phys}/\hbar$。指数宗量必须无量纲，因而在本书 $\hbar=1$ 的记法中

$$
[S]=0,\qquad 0=[d^dx]+[\mathcal L]=-d+[\mathcal L],
\qquad [\mathcal L]=d.
\tag{C12.06}\label{eq:c12-action-dimension}
$$
...
\tag{C12.07}\label{eq:c12-field-dimension}
$$
...
$$
[J]=\frac{d+2}{2}.
\tag{C12.08}\label{eq:c12-source-dimension}
$$
...
[g_n]=d-\frac n2(d-2).
\tag{C12.09}\label{eq:c12-coupling-dimension}
$$
...
\tag{C12.10}\label{eq:c12-cubic-dimension}
$$

在 $d=4$ 时，立方耦合有一个质量单位，与第9至11章的计算一致；在 $d=6$ 时，它成为无量纲量。用同一个公式计算其他幂次，可以直接比较两种时空维数中的结果：
...
\tag{C12.11}\label{eq:c12-derivative-coupling}
$$

例如[习题10.4](#ex10-4)的 $\varphi(\partial\varphi)^2$ 在四维中有三个场和两个导数，其系数维数为 $4-2-3=-1$。它与无导数立方势的系数都曾用 $g$ 表示，使用时仍须分别按相应的量纲计数。

## 高能处真正参与展开的参数 {#c12-high-energy}

耦合的维数影响它怎样进入振幅。为此分别考察耦合质量维数为正、负和零的情形。令硬能量尺度为 $Q=\sqrt s$，耦合维数为 $\kappa=[g]$；用能量的适当幂消去单位后，参与比较的无量纲参数为

$$
\widehat g(Q)=gQ^{-\kappa}=g\,s^{-[g]/2}.
\tag{C12.12}\label{eq:c12-dimensionless-coupling}
$$
...
\end{aligned}
\tag{C12.13}\label{eq:c12-amplitude-dimension}
$$

最后一步代入 $2I=nV-N_{\rm ext}$ 和式\eqref{eq:c12-coupling-dimension}，全部 $V$ 项相消，所以同一外腿数的图无论处于哪一阶，都有相同的振幅维数。
...
\tag{C12.14}\label{eq:c12-amplitude-scaling-form}
$$

前面的能量幂由量纲固定，函数 $F$ 的具体形式则由动力学计算决定。若理论还有其他质量或耦合，就须加入相应的无量纲比值；使用调节尺度 $\Lambda$ 时，也要保留 $\Lambda/Q$。第11章的四维树幅就是一个例子：远离前、后向的角域以 $g^2/s$ 为小量，前向峰附近的交换过程却仍受 $m^2-t$ 控制。

先将 $g$ 的数值固定，只考察这种工程能量幂。若 $\kappa=-p<0$，定义 $\Lambda_g=|g|^{-1/p}$，就有

$$
|\widehat g(Q)|=|g|Q^p=\left(\frac Q{\Lambda_g}\right)^p.
\tag{C12.15}\label{eq:c12-negative-dimension-scale}
$$

在 $Q\ll\Lambda_g$ 时，无量纲参数可以很小；随着能量升至 $\Lambda_g$ 附近，这种微扰小量便不再存在。这一性质与负维耦合的微扰不可重整化性相联系，详细论证见[第18章](#c18)。
...
\tag{C12.16}\label{eq:c12-superficial-degree-bridge}
$$

这里先限于无导数顶点，并将所有独立圈动量同时放大；只有部分动量变大的子区域，还须分别检查。若 $[g_n]<0$，固定外腿数时 $\omega$ 会随顶点数增加，因而允许出现越来越高次的外动量局部项。原有有限个相互作用项通常不足以吸收所有阶的短距离贡献，第18章将由实际圈积分确定所需反项，第29章再讨论怎样按有效理论组织它们。

要求任意高阶精度时，一般需要不断增加输入参数。若只要求固定低能精度，可以按算符维数截断。例如六维实标量满足 $[\varphi]=2$，保留维数不超过 $D_{\max}$ 的局部项，就要求 $2n+r\le D_{\max}$。非负整数 $n,r$ 只有有限种取值，每种对应的有限多个Lorentz指标也只有有限种收缩，因此在固定截断内只需有限个系数。提高精度时再依次加入更高维项，便形成第29章的低能展开。

若 $\kappa>0$，则 $\widehat g(Q)\to0$，相互作用在固定硬运动学的工程幂意义下趋弱。第11章的前、后向峰提醒我们，实际过程仍须按相应动量区域判断。若 $\kappa=0$，便有 $\widehat g=g$，量纲本身不再要求一个随能量增长或衰减的幂。量子修正仍可产生对数依赖，微扰在各能量处是否适用还需由后续计算决定。
...
[J]=4,\qquad [g_3]=0.
\tag{C12.17}\label{eq:c12-six-dimensions}
$$

这个选择保留了最简单的三价顶点，同时使耦合无量纲，因而适合用来展开后续重整化计算。

时空维数改变以后，模式归一化和积分测度也要一同改变。将正能模式及Fourier表示延伸到 $d$ 维，得到
...
\end{aligned}
\tag{C12.18}\label{eq:c12-dimensional-measures}
$$

这里仍沿用第3、8章的归一化，只是每个空间方向各提供一个动量delta及相应积分。位置传播核因此有 $[\Delta(x)]=d-2$，动量核仍为质量平方的倒数。由式\eqref{eq:c12-amplitude-dimension}，六维四腿振幅的维数为 $-2$。

第11章相空间中 $16\pi^2$ 等具体系数来自三维空间的球坐标积分，继续适用于其四维问题；六维问题则要重新计算五维空间的角度积分。即使暂未完成这些积分，仍可先确定可观测量的量纲：$n$ 体相空间的维数为 $n(d-2)-d$，分别与两入射幅及通量、单入射幅及能量分母结合，得到
...
+n(d-2)-d-1=1.
\end{aligned}
\tag{C12.19}\label{eq:c12-observable-dimensions}
$$

所以六维截面的量纲是长度的四次方，衰变率仍是时间的倒数。[习题12.1](#ex12-1)再通过 $\hbar c$ 的具体换算，将自然单位中合并的长度与能量单位接回普通单位。
