...
本章先从经典势和量子力学双阱看清真空选择的含义，再平移场以描述选定真空附近的粒子，最后用第21章的量子作用量说明：平移虽然产生了新的顶点，却不需要增加独立的重整化参数。四维中取$\lambda>0$，并用$v_0$标记树级最低点、$v$标记含圈修正的真空值，以区分两步计算。

## 两个经典最低点 {#c30-classical}
...
\tag{C30.01}\label{eq:c30-model}
$$

势和动能都在场的符号反射下不变，因此保留第23章的离散对称性。在包含所需态的表示中，用幺正算符实现这一变换，并取

$$
Z^{-1}\varphi(x)Z=-\varphi(x),\qquad Z^2=1 .
\tag{C30.02}\label{eq:c30-discrete-symmetry}
$$
...
\tag{C30.03}\label{eq:c30-completed-square}
$$
...
寻找场论的最低能量还要考虑动能和空间梯度。移去常数后，Hamiltonian为

$$
H=\int d^3x\left[
\frac12\dot\varphi^2+\frac12(\boldsymbol\nabla\varphi)^2
+\frac{\lambda}{24}(\varphi^2-v_0^2)^2\right].
\tag{C30.04}\label{eq:c30-positive-energy}
$$
...
\tag{C30.05}\label{eq:c30-curvatures}
$$

原点的曲率为负，而两个最低点的曲率均为$-2m^2>0$，所以小涨落应围绕后两者展开。若$m^2>0$，原点就是唯一的经典最低点，回到前面熟悉的情形。

## 双阱量子力学与体积极限 {#c30-vacuum-limit}

经典势有两个最低点，并不立即意味着量子理论也有两个可分别选择的基态。量子力学中的双阱提供了一个熟悉的例子。考虑单位质量粒子的Hamiltonian

$$
H_{\rm QM}=\frac12p^2+\frac{\lambda}{24}(x^2-v_0^2)^2.
\tag{C30.06}\label{eq:c30-double-well}
$$
...
\tag{C30.07}\label{eq:c30-local-gaussians}
$$

这两个Gaussian的典型宽度为$\omega^{-1/2}$。当$\omega v_0^2\gg1$时，宽度远小于中心到原点的距离，三次、四次项相对二次项分别按$|y|/v_0$和$y^2/v_0^2$受控，这就是局部简谐近似的适用条件。
...
\int dx\,e^{-\omega x^2}
=e^{-\omega v_0^2}.
\tag{C30.08}\label{eq:c30-gaussian-overlap}
$$

反射对称的Hamiltonian会混合这两个局域态，因此在两态近似中应取偶、奇线性组合。计入非零重叠，它们的归一化形式为

$$
|e\rangle=\frac{|+\rangle+|-\rangle}{\sqrt{2(1+s_0)}},
\qquad
|o\rangle=\frac{|+\rangle-|-\rangle}{\sqrt{2(1-s_0)}}.
\tag{C30.09}\label{eq:c30-even-odd-combinations}
$$

有限双阱的精确基态确实是偶态，这可以从一维Schrödinger方程看出。先将基态取为实函数，再取其绝对值，动能和势能都不会增加，因而仍达到最低能量。这个非负解若在内部某点为零，该点的一阶导数也必须为零；微分方程解的唯一性就会迫使它处处为零，与归一化矛盾，所以基态严格为正。再考察两个同能衰减解，它们的Wronskian是常数，并在无穷远趋于零，故两解线性相关。反射后的正解于是只能等于原基态，基态为偶态。最低奇态的能量稍高，阱间隧穿造成这一小的能级分裂，而式\eqref{eq:c30-even-odd-combinations}给出两者在深阱时的近似形式。
...
\tag{C30.10}\label{eq:c30-volume-zero-mode}
$$

均匀模的两个势阱中心位于$\pm\sqrt{\mathcal V}v_0$，随体积增加而彼此远离，但小振动频率仍是$M_0=\sqrt{\lambda v_0^2/3}$。将这一中心和频率代入式\eqref{eq:c30-gaussian-overlap}，重叠便成为

$$
s_{\mathcal V}=e^{-M_0v_0^2\mathcal V}
\ \longrightarrow\ 0\qquad(\mathcal V\to\infty).
\tag{C30.11}\label{eq:c30-volume-overlap}
$$

如果在两个阱附近采用相同的场二次核，常数位移只改变零动量模，其余模式的Gaussian内积都为1。因此，场波泛函的重叠正由上述零模决定。指数中的$M_0v_0^2\mathcal V$是无量纲量，显示压低来自物理空间体积的增长；在固定体积中增加格点数，并不是这里取的极限。

正交性的理由还可以摆脱Gaussian形状，只要已有两个满足聚类条件的纯相。先固定局部UV平滑，考察区域$\mathcal R$中的平均场$\bar\varphi_{\mathcal R}=\mathcal V_{\mathcal R}^{-1}\int_{\mathcal R}d^3x\,\varphi(x)$。若两相平均值分别趋于$\pm v$、$v>0$，而各自连通的等时关联$C_\pm(r)$可积，宏观平均场的方差就满足
...
\longrightarrow0.
\tag{C30.12}\label{eq:c30-cluster-variance}
$$

因此，区域增大时，平均场在每个相中都越来越集中于自身的期望值。将有限体积态的这两个平均值记作$v_+$、$v_-$，在$(v_+-v_-)\langle+|-\rangle$中加减同一个平均场，再用Cauchy–Schwarz不等式，就能把态的重叠与这两个方差联系起来：
...
\longrightarrow0 .
\end{aligned}
\tag{C30.13}\label{eq:c30-orthogonality-bound}
$$

由于$v_+-v_-\to2v\ne0$，方差趋零就迫使两相的重叠趋零。这里用的是两种纯相已经存在并满足聚类条件；在本章弱耦合、远离临界点的破缺相中，有质量的小涨落正与这些条件相配。

要实际选出其中一个相，可以加入很小的恒定实源$j$。Lorentz拉格朗日量中的$+j\varphi$对应$H_j=H-j\int d^3x\,\varphi$，所以正$j$使正相的能量较低。先作基态投影，再固定$j$取无限空间体积，最后撤去源，分别定义
...
\tag{C30.14}\label{eq:c30-order-of-limits}
$$

极限次序在这里起着实质作用：若在有限对称盒中先令$j=0$，就会回到偶基态及零一点函数。也可以不用微小体源，而用固定正、负边界分别选出对应的相。

将两个真空记作$|0+\rangle,|0-\rangle$，它们满足$\langle0\pm|\varphi(x)|0\pm\rangle=\pm v$，当前弱耦合近似中有$v=v_0+\hbox{圈修正}$。对称算符互换两态时一般可带相位：若$Z|0+\rangle=e^{i\alpha}|0-\rangle$，则$Z^2=1$使反向相位为$e^{-i\alpha}$。调整负态的整体相位，即可写成$Z|0+\rangle=|0-\rangle$。这与对称真空有根本区别：若正真空在$Z$下仅乘一个相位，式\eqref{eq:c30-discrete-symmetry}就会要求$v=-v$，所以$v\ne0$的真空不能单独保持反射对称性。在同时包含两相的空间中，$Z$将它们互换；限制到一个无限体积纯相时，这一变换把态送到另一相的表示。

## 在选定真空附近建立粒子理论 {#c30-shift}
...
\tag{C30.15}\label{eq:c30-shift-polynomial}
$$
...
\tag{C30.16}\label{eq:c30-shifted-lagrangian}
$$
...
$$
Z^{-1}\rho Z=-\rho-2v_0 .
\tag{C30.17}\label{eq:c30-affine-symmetry}
$$
...
\tag{C30.18}\label{eq:c30-vacuum-shift}
$$

所以配平方得到的树关系$v_0^2=-6m^2/\lambda$仅确定最低阶位置，精确的$v$还含圈修正。下一章将显式计算圈修正；这里先考察一点条件怎样联系各个反项。

## 为什么仍只有三个独立反项 {#c30-counterterms}

平移产生的新顶点会不会要求新的独立参数？先把三个重整化常数放回对称形式：

$$
\mathcal L=-\frac12Z_\varphi(\partial\varphi)^2
-\frac12Z_m m^2\varphi^2-\frac{Z_\lambda\lambda}{24}\varphi^4 .
\tag{C30.19}\label{eq:c30-symmetric-counterterms}
$$
...
\end{aligned}
\tag{C30.20}\label{eq:c30-full-shift}
$$

所有系数都来自原来的二次和四次多项式，新增的线性、三次项因而已经受同一对称结构约束。把这些关系改写成反项形式会更直观。记$A=Z_\varphi-1$、$B=Z_m-1$、$C=Z_\lambda-1$，暂在$v_0$处平移，利用$\lambda v_0^2/6=-m^2$，再减去树级式\eqref{eq:c30-shifted-lagrangian}，得到场依赖反项
...
\end{aligned}
\tag{C30.21}\label{eq:c30-shifted-counterterm-polynomial}
$$
...
\tag{C30.22}\label{eq:c30-tadpole-condition}
$$

后两项合为$U'_{1,\rm ren}$，就恢复式\eqref{eq:c30-vacuum-shift}的真空位移方程。因此$\delta v_1$是在已有参数下确定真空位置，并未引入可独立调节的三次物理耦合。

为与下一章的显式积分衔接，先写出蝌蚪图的组合因子。三价相互作用带$1/3!$，选择哪一条腿作外腿有三种方式，乘起来留下$1/2$，其余两场相互收缩。按本书的顶点和传播线相位，一点图为
...
\tag{C30.23}\label{eq:c30-tadpole-factor}
$$

这一图应与$-i[m^2v_0(B-C)+M_0^2\delta v_1]$相加为零，正与$\Gamma^{(1)}=-U'$的负号一致。积分中先保留UV调节；下一章将延拓到$d=4-\epsilon$，补入$\lambda_d=\lambda\widetilde\mu^\epsilon$，评价蝌蚪积分后再与二、三、四点图比较。

上述多项式关系说明了平移反项如何联系，而三个独立反项足够的原因，则在于原理论的局域UV结构。对四维四次图，端口计数$4V=2I+E$及圈数关系$L=I-V+1$给出

$$
\omega=4L-2I=4-E .
\tag{C30.24}\label{eq:c30-counterterm-power-count}
$$

减去所有子图发散后，二点图剩余的新发散至多是外动量的二次多项式，四点图至多是常数，更多外腿的图没有新的整体发散。再要求Lorentz不变性及$\varphi\mapsto-\varphi$对称性，场依赖局域反项就只有$(\partial\varphi)^2$、$\varphi^2$和$\varphi^4$。这是[第18章的子图减除](#c18-forest)在本模型中的应用，场无关的真空常数仍另行减去。一般$d\le4$时，$[\lambda]=4-d\ge0$，同一幂计数仍给出可重整化的范围。常数平移只是重新排列这些局域多项式，故不会增加独立UV参数。
...
## 对称的量子作用量与真空方程 {#c30-quantum-action}

要把反项的联系与真空条件放在同一个对象中，就回到第21章的量子作用量。由全部1PI核组成的泛函为
...
\end{aligned}
\tag{C30.25}\label{eq:c30-quantum-action}
$$

这里$\int_k=\int d^4k/(2\pi)^4$。二次核前的负号沿用第21章的Legendre变换约定；在树级，$V_4=-\lambda$，高点部分也给出负的四次项，于是$\Gamma$回到经典作用量。

在对称真空处，四价点与二价反项的端口计数满足$4V_4+2V_2=2I+E$，因此$E$必为偶数：奇数外腿的$V_n$为零，$\Gamma$是偶泛函。这也可以从[习题21.2](#ex21-2)的变量替换直接理解。在保持反射的有限调节、积分域和边界下，$\chi\mapsto-\chi$的绝对Jacobian为1，逐步得到

$$
Z[J]=Z[-J],\quad W[J]=W[-J],\quad
\varphi[-J]=-\varphi[J],\quad \Gamma[-\varphi]=\Gamma[\varphi].
\tag{C30.26}\label{eq:c30-symmetry-inheritance}
$$

最后一步使用了$W-\int J\varphi$在源和场同时变号时不变。若边界已经选定正相，反射也会把边界变为负相，此时等式联系的是两相对应的分支。这个换元同时使用了经典作用量与积分测度的对称性。若调节后不能同时保持相应对称，就会出现反常；本例的实场符号反射保持有限模式测度，反常的具体情形将在第75章讨论。
...
+\hbox{更多导数项}\right].
\tag{C30.27}\label{eq:c30-derivative-action}
$$

寻找平移不变真空时，平均场为常数，全部导数项消失。因此，第21章的$\delta\Gamma/\delta\varphi=-J$直接化为

$$
-U'(\varphi_j)=-j,\qquad U'(\varphi_j)=j,\qquad U'(v)=0 .
\tag{C30.28}\label{eq:c30-quantum-vacuum-equation}
$$
...
\tag{C30.29}\label{eq:c30-curvature-and-pole}
$$

这里$U''$给出零动量曲率，$U''/Z$则是保留到二阶导数时的近似质量平方。物理质量由完整核的极点决定，一般还受到更高动量项的影响，相应留数为$[D'_\rho(-M_{\rm pole}^2;v)]^{-1}$。因此，树级从二次势识别质量的做法，到下一章计入圈修正时，要转为对完整二点核求极点。

## 关于全局量子势的一点补充 {#c30-convex-potential}

上面的弱耦合计算在两个等能最低点附近分别展开，适合描述各个纯相中的涨落。若考察完整的全局Legendre势，还要把两相共存计入，所得势具有凸性。为了看清这一性质与前面双阱图像的联系，先从有限Euclidean积分出发，保留正测度、有限UV调节和有限四体积$\Omega$，并采用正源记号

$$
Z_E(j)=\int D\chi\,e^{-S_E+j\int_\Omega\chi},\qquad
w_\Omega(j)=\frac1\Omega\ln Z_E(j),\qquad
\bar\chi=\frac1\Omega\int_\Omega\chi .
\tag{C30.30}\label{eq:c30-euclidean-convexity-start}
$$

这里的$j$是第29章负欧氏源的相反数。对源的每次微分都插入一个$\int_\Omega\chi$，因此对数生成函数的一阶导数是平均场，二阶导数是四体积乘以方差：

$$
w_\Omega'(j)=\langle\bar\chi\rangle_j,\qquad
w_\Omega''(j)
=\Omega\left(\langle\bar\chi^2\rangle_j-\langle\bar\chi\rangle_j^2\right)\ge0.
\tag{C30.31}\label{eq:c30-source-variance}
$$

为了把固定源的描述改写为固定平均场的描述，取完整的Legendre变换，定义全局势

$$
U_\Omega(\phi)=\sup_j\{j\phi-w_\Omega(j)\}.
\tag{C30.32}\label{eq:c30-legendre-supremum}
$$
...
\tag{C30.33}\label{eq:c30-flat-coexistence}
$$
