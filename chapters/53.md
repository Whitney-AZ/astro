...
可以通过把它展开成Feynman图看清楚。

我们先讨论复标量，再讨论Dirac场；两个计算的差别将给出闭费米圈的统计负号。[第71章](#c71)计算非阿贝尔规范场的路径积分时还会用到这项技术，先在熟悉的高斯理论中把它建立起来。

## 在指定背景中积掉复标量 {#c53-scalar-gaussian}
...
\tag{C53.01}\label{eq:c53-scalar-model}
$$
...
稍后Dirac例子也记耦合为$g$，但那时它是无量纲的另一个耦合。
...
=\frac{\displaystyle\int\mathcal D\chi^\dagger\mathcal D\chi\,
       ...
       {\displaystyle\int\mathcal D\chi^\dagger\mathcal D\chi\,
                e^{iS_\chi[0]}},
\qquad Z_\chi[0]=1 .
\tag{C53.02}\label{eq:c53-scalar-normalization}
$$

分子和分母使用同一个测度、边界条件及调节。
可先限制在有限区域和有限基底中，使积分成为普通的多变量积分，
再取相应的Feynman真空边界极限。下面写出的连续核，是这套矩阵运算的简记。
...
\tag{C53.03}\label{eq:c53-integration-by-parts}
$$

在共同的周期边界或适当衰减条件下，最后一项为零；
采用真空边界定义时，也须在分子分母中一致地处理它。
因此二次型的核可以取为
...
\tag{C53.04}\label{eq:c53-scalar-kernel}
$$

前两章的标量质量$M$与本章二次核$Q$是不同对象；自由核提出后的余因子将记为$\widetilde Q$。

在有限基底中设$\chi$有$N$个复坐标$z_j$。
沿第44章的测度，$z_j=(x_j+iy_j)/\sqrt2$，
$d^Nz\,d^N\bar z=d^Nx\,d^Ny$。
[普通复高斯积分](44.md#c44-complex-gaussian)已给出
...
\tag{C53.05}\label{eq:c53-finite-scalar-gaussian}
$$
...
\tag{C53.06}\label{eq:c53-scalar-determinant-ratio}
$$
...
\end{gathered}
\tag{C53.07}\label{eq:c53-operator-factor}
$$

这一写法先要求自由核可逆。其逆取Feynman传播子$\Delta$，
沿本书定义满足
...
\end{aligned}
\tag{C53.08}\label{eq:c53-remainder-kernel}
$$
...
\end{aligned}
\tag{C53.09}\label{eq:c53-factor-in-kernels}
$$

第二行的导数只作用于$\Delta(x-z)$，最后一步才用
$\varphi(z)\delta^4(x-z)=\varphi(x)\delta^4(x-z)$。
这样就得回式\eqref{eq:c53-scalar-kernel}，背景也保持为局部乘法算符。
...
\tag{C53.10}\label{eq:c53-normalized-scalar-det}
$$
...
## 从矩阵对数到闭合传播链 {#c53-trace-log}

要看清行列式里包含什么，最方便的是先取对数。
所需关系是$\det A=\exp(\operatorname{Tr}\ln A)$，
可在Jordan基底中证明。对于一个可逆的有限矩阵，
一个Jordan块可写成$J=aI+N$，其中$a\ne0$且$N^r=0$。
选定$a$的对数分支后，
...
\det J=a^r .
\tag{C53.11}\label{eq:c53-jordan-log}
$$

$N$及其正整数次幂都严格上三角，迹为零，
所以只有对角部分进入$\operatorname{Tr}\ln J$。
对全部Jordan块相加，再用相似变换不改变迹与行列式，便得到
...
\tag{C53.12}\label{eq:c53-scalar-log-series}
$$
...
\ln\det(I-G_\chi)
 ...
 =-\sum_{n=1}^{\infty}\frac{\operatorname{Tr}G_\chi^n}{n}.
\end{aligned}
\tag{C53.13}\label{eq:c53-resolvent-log}
$$

第一个等式可在相邻两点间分解
$I-(t+h)G_\chi=(I-tG_\chi)
[I-h(I-tG_\chi)^{-1}G_\chi]$后取行列式得到。
在$\|G_\chi\|<1$时，Neumann级数在$0\leq t\leq1$上一致收敛，
...
\qquad x_{n+1}=x_1 .
\end{aligned}
\tag{C53.14}\label{eq:c53-scalar-trace-chain}
$$
...
\begin{figure}[htbp]
\centering
\input{notes/figures/s53_background_ring.tex}
\renewcommand{\thefigure}{C53a}
\caption{背景中的有向圈。黑点给出$ig\varphi(x_j)$，
箭头从传播核的第二坐标指向第一坐标；
右侧省略了其余插入点。四个显式点代表一般圈的一部分，
不把插入数限定为四。两种量子场的内线分别为$\Delta/i$和$S/i$。}
\label{fig:c53-background-ring}
\end{figure}
...
\tag{C53.15}\label{eq:c53-scalar-wick-permutations}
$$
...
\end{aligned}
\tag{C53.16}\label{eq:c53-scalar-connected-weight}
$$

第一行的$n$个顶角各带一个$i$，$n$条传播线各带一个$1/i$，
二者完全相消。积掉所有坐标时，
$\prod_j\varphi(x_j)=\prod_j\varphi(x_{j+1})$，
于是第二行正好成为式\eqref{eq:c53-scalar-trace-chain}的迹。
...
因此[第9章的连通图指数关系](09.md#c09-connected)在这里具体成为
...
\tag{C53.17}\label{eq:c53-disconnected-exponentiation}
$$
...
\tag{C53.18}\label{eq:c53-scalar-induced-action}
$$

行列式把所有背景插入数一次包含在内；取它的对数，则恰好留下连通的圈。

## 积掉Dirac场 {#c53-dirac-gaussian}

接着把被积变量换成Dirac场，仍保持$\varphi(x)$为指定背景。
这里采用普通标量耦合：
...
\end{aligned}
\tag{C53.19}\label{eq:c53-dirac-model}
$$
...
若共有$N$个时空基函数，就有$4N$对独立的Grassmann变量。
第44章的[复Grassmann高斯积分](44.md#c44-determinant)
...
\qquad Z_\Psi[0]=1 .
\end{aligned}
\tag{C53.20}\label{eq:c53-dirac-determinant-ratio}
$$

Grassmann积分只取有限多项式的最高次系数，因而这里不需要普通高斯积分的收敛条件。
...
\end{aligned}
\tag{C53.21}\label{eq:c53-dirac-kernels}
$$
...
\end{aligned}
\tag{C53.22}\label{eq:c53-dirac-factor-check}
$$
...
\end{aligned}
\tag{C53.23}\label{eq:c53-dirac-trace-log}
$$
...
\end{aligned}
\tag{C53.24}\label{eq:c53-dirac-trace-chain}
$$
...
## 费米子闭圈的负号 {#c53-fermion-sign}

Dirac图也只有双线性插入，所以循环起点的计数仍给$1/n$。
...
\tag{C53.25}\label{eq:c53-dirac-before-statistics}
$$

它与式\eqref{eq:c53-dirac-trace-log}比较，还差一个整体负号。
行列式已经计入了Grassmann积分的统计效应，而刚才的图因子尚未计入。
...
\langle T\prod_{j=1}^n B_jA_j\rangle_0
 &=(-1)^n
   \sum_{\pi\in\mathfrak S_n}\operatorname{sgn}\pi
       \prod_{j=1}^n K_{j,\pi(j)} .
\end{aligned}
\tag{C53.27}\label{eq:c53-fermion-wick-permutations}
$$
...
\tag{C53.28}\label{eq:c53-one-minus-per-cycle}
$$

每个循环就是一个闭圈，所以每个闭费米圈贡献一个负号，
而与圈上有多少个顶点无关。
对于一个连通圈，将这个号与式\eqref{eq:c53-dirac-before-statistics}相乘，
便有
...
\end{aligned}
\tag{C53.29}\label{eq:c53-dirac-connected-weight}
$$
...
这样，Grassmann配对、行列式正幂与图的闭圈规则得到了同一个答案。
本章未对相互作用作正规序；普通标量耦合允许蝌蚪及其它奇数插入项，
因此不能在圈和中自行删去奇数$n$。

## 背景的量子作用量与紫外项 {#c53-action}
...
\tag{C53.30}\label{eq:c53-induced-action-in-path-integral}
$$

所以$\Gamma$直接增加到$\varphi$的作用量中。
只要被积场在固定背景下仍是二次型，这次高斯积分就是精确的。
它含有任意多个背景插入，但每个连通图都只有一个被积场的圈；
$\varphi$自身的量子涨落则由余下的路径积分产生。
这与第21章通过Legendre变换定义完整有效作用量的步骤有所区别：
此处只完成了对其中一种场的积分。

还须处理圈的紫外发散。归一条件$Z[0]=1$消去了无背景的自由真空因子，
但有背景插入的圈一般仍然发散。
以固定外动量考察大的Euclidean圈动量$\ell$，
标量传播子按$\ell^{-2}$衰减，Dirac传播子的上界按$\ell^{-1}$衰减，
所以$n$个插入的表面发散度为

$$
\omega_{\chi,n}=4-2n,\qquad
\omega_{\Psi,n}=4-n .
\tag{C53.31}\label{eq:c53-background-power-counting}
$$

复标量的一次插入可产生线性背景项，二次插入可产生背景平方项。
Dirac圈还可产生三次、四次背景项，二点圈对外动量的二次展开则产生动能项。
...
各自的系数须按所选调节和减除条件确定。
本章的圈展开应先在这套共同调节下使用，再作相应的局部减除。
行列式给出的是组织整个计算的方法，归一化本身没有代替重整化。

## 补充：实场与Majorana场的二分之一 {#c53-half-weights}

两个例子都使用复变量。与第44章的实变量积分比较，还能看清
实标量和Majorana场的圈权重为什么各带一个二分之一。
...
结合[实高斯积分](44.md#c44-real-gaussian)，在零背景附近选择相同的连续分支，
...
\tag{C53.32}\label{eq:c53-real-half-weight}
$$
...
对于Majorana场，沿前文约定$\bar\Psi=\Psi^T\mathcal C$，
...
| Dirac场    | $-\operatorname{Tr}G_\Psi^n/n$    |
| Majorana场 | $-\operatorname{Tr}G_\Psi^n/(2n)$ |

普通积分与Grassmann积分决定统计号，实自由度与复自由度的数目决定半因子。
