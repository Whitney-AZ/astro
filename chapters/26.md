...
第20章在六维$\varphi^3$理论中算出的高能振幅含有$\ln(s/m^2)$；这个对数在固定碰撞能量、质量趋于零时越来越大。大圈动量引起的发散已经经过重整化，这个小质量极限中的问题提示我们，还应检查所算的“两个粒子散射”与实验事件之间的对应。如果探测器把两个几乎同向的粒子记录成一个，精确两粒子末态的截面就没有包含所有能给出同一读数的过程。本章从这种有限分辨率出发，计算遗漏的真实分裂，并将它与虚圈修正相比较。为控制共线区域的积分，质量$m$先保留非零，最后再考察小质量极限。

## 大对数与未分辨的粒子 {#c26-observable}

先回到$s,|t|,|u|\gg m^2$且$t/s,u/s$固定的高能区域。此时第20章三个通道中的对数可以分成共同的质量对数和固定散射角的部分，振幅因而写成
...
\end{gathered}
\tag{C26.01}\label{eq:c26-hard-log}
$$

例如，第20章的$L_t$可写成$L+\ln(|t|/s)$，而$L_s=L-i\pi$。固定角比值的对数以及$i\pi$在$m\to0$时均有限，属于$O(m^0)$。这个记号汇集了不含发散质量对数的部分，其中的有限项仍可在需要时保留。重组各通道的对数时，仍须沿用第20章已经确定的物理分支。

这种质量极限也影响了对外态的理解。第5章隔离单粒子态时，曾使用单粒子与多粒子连续谱之间的间隔；质量趋于零后，几个同向粒子的总不变质量也可以趋于零，原来的隔离条件便失去保障。对应到测量，能量很低的粒子可能不被记录，夹角很小的两个粒子也可能合成一个读数。前者称为软（soft）区域，后者称为共线（collinear）区域。初态中若还有未识别的粒子，也应根据实际制备方式把它们计入。
...
\tag{C26.02}\label{eq:c26-measured-rate}
$$
...
本章取两个彼此分离的硬方向，并规定：若一对子动量的夹角$\theta<\delta$，就将它们的空间动量合并，计入同一个硬动量区间。这个区间应容纳小的共线反冲，才能在共线近似下提出公共的硬相空间。提出这一公共部分后，将未分辨末态的概率和记为$|\mathcal T|_{\rm obs}^2$，便有
...
\tag{C26.03}\label{eq:c26-inclusive-sum}
$$

式中省写了其他末态的公共测度、通量和总delta函数，第二项在未分辨区域内积分。两子粒子相同，完整的带标签积分会把同一个无序对子计数两次，因此出现$1/2$；顶点中的收缩阶乘已经包含在三价费曼规则内。

## 分裂振幅和红外幂计数 {#c26-singular-regions}

现在计算第二项中的分裂振幅。图C26a在原来的一条出射硬腿上接入一个三价顶点，原来的外线变成一条内部传播子。把这个传播子和新增顶点乘到硬过程图块上，得到
...
\tag{C26.04}\label{eq:c26-splitting-factor}
$$
...
\begin{figure}[htbp]
\centering
\input{notes/figures/c26*split.tex}
\renewcommand{\thefigure}{C26a}
\caption{一条出射腿的共线分裂。灰圆包含硬过程的图之和，
并非裸四点顶角。内部总动量$k*{\rm pair}$流入显式顶点，再分成$k_1,k_2$；
箭头表示动量，不表示实标量的电荷。}
\label{fig:c26-split}
\end{figure}
...
\tag{C26.05}\label{eq:c26-massless-virtuality}
$$
...
\theta^{d-7}d\theta.
\tag{C26.06}\label{eq:c26-ir-power-counting}
$$

因为$\int_0^\epsilon y^pdy$仅在$p>-1$时收敛，软能量下限在$d\le4$时发散，共线角下限则在$d\le6$时发散。因此，对本章的六维模型，纯软区域仍可积，留下的是共线对数。下面将以探测器的小角锥为积分区域，求出这一对数的系数。
...
+O(\omega_q^0).
\tag{C26.07}\label{eq:c26-soft-sum}
$$
...
\right].
\tag{C26.08}\label{eq:c26-soft-interference}
$$
...
\tag{C26.09}\label{eq:c26-cluster-resolution}
$$
...
\tag{C26.10}\label{eq:c26-two-cluster-momenta}
$$
...
\tag{C26.11}\label{eq:c26-exact-phase-factorization}
$$
...
在共线区域内，硬振幅和测量权重可以按$M_{\rm pair}^2/s$展开。提出它们的领先项后，再用式\eqref{eq:c26-cluster-resolution}分离公共硬腿测度，得到单腿修正：
...
{(k_{\rm pair}^2+m^2)^2}.
\tag{C26.12}\label{eq:c26-local-splitting-probability}
$$
...
\tag{C26.13}\label{eq:c26-five-dimensional-measure}
$$

这里$\Omega_4$表示横向四维空间中$S^3$的面积；上一章的$\Omega_5$则是五维动量空间的全角面积。两者对应不同的角积分，不应混用。
...
\tag{C26.15}\label{eq:c26-sine-geometry}
$$
...
\tag{C26.16}\label{eq:c26-collinear-fractions}
$$

从$(r_1,\beta)$换到$(x,\theta)$时，径向动量和极角都依赖两个新变量，因此要同时计算二维变量代换的Jacobian。先对精确正弦关系求导，得到行列式
...
\end{aligned}
\tag{C26.17}\label{eq:c26-geometric-jacobian}
$$

取其绝对值并在小角处展开，积分所需的Jacobian为$Kx[1+O(\theta^2)]$。将它连同式\eqref{eq:c26-collinear-fractions}一起代入测度，并在传播子分母以外保留领先质量项，各个因子依次合成为
...
\end{aligned}
\tag{C26.18}\label{eq:c26-measure-jacobian-product}
$$

其中最后一份$x$来自变量代换的Jacobian，它参与决定后面能量份额积分的权重。
...
\tag{C26.19}\label{eq:c26-exact-pair-denominator}
$$
...
\tag{C26.20}\label{eq:c26-denominator-expansion}
$$
...
\tag{C26.21}\label{eq:c26-mass-regulated-denominator}
$$

以等分动量$x=1/2$、$\theta=0$为例，精确分母为$-3m^2$，这个展开式也给出$-3m^2$。质量项因此使传播子分母保持非零，其影响主要集中在$\theta^2\lesssim(m^2/K^2)f(x)$的区域，正好截住了无质量情形的共线增强。

现在将式\eqref{eq:c26-measure-jacobian-product}除以传播子分母平方，公共的$K^4x^2(1-x)^2$相消，动量份额权重只留下$x(1-x)$。六维的整体归一化也可用圈展开参数表示，因为
...
\tag{C26.22}\label{eq:c26-double-integral}
$$
...
\end{aligned}
\tag{C26.24}\label{eq:c26-exact-angular-integral}
$$
...
\tag{C26.25}\label{eq:c26-angular-asymptotic}
$$
...
\tag{C26.26}\label{eq:c26-log-moments}
$$

第二式先用$x\leftrightarrow1-x$将两份对数合并，再由$\int_0^1x^n\ln x\,dx=-1/(n+1)^2$得到$2(-1/4+1/9)=-5/18$。第三式已经在第14章的式\eqref{eq:c14-j1-evaluated}中求出：本章的$w$与当时的Feynman组合$x(1-x)$相同，因而直接代入那个积分结果即可。最后，由$f=(1-w)/w^2$把两种对数合起来，有
...
\tag{C26.27}\label{eq:c26-f-log-moment}
$$
...
\end{aligned}
\tag{C26.28}\label{eq:c26-collinear-constant}
$$
...
+\text{小角与小质量的高阶项}.
\tag{C26.29}\label{eq:c26-single-leg-result}
$$

其中$c$取两位小数时为$-4.11$。

上面先使用了式\eqref{eq:c26-angular-asymptotic}，再对$x$积分，因此还要看渐近式在端点附近的误差。当$x$接近0或1时，$f(x)$发散，条件$a\ll\delta^2$不再对整个区间一致成立。令$r=m^2/(K^2\delta^2)$，将精确角积分与式\eqref{eq:c26-angular-asymptotic}前三项相减，余项可直接写成

$$
E_r(x)=\frac12\left[
\ln(1+rf(x))+\frac{rf(x)}{1+rf(x)}
\right]\ge0.
\tag{C26.30}\label{eq:c26-endpoint-remainder}
$$
...
\tag{C26.31}\label{eq:c26-endpoint-bound}
$$

因此，固定$x$得到的小质量展开在加权积分后仍保留正确的对数和有限常数。

分母外的能量展开也应先在$\eta<x<1-\eta$上使用，再让$\eta$趋于零。对于$r_i=O(m)$的极端软端点，要保留精确能量$\omega_i=\sqrt{r_i^2+m^2}$。例如，$x\lesssim m/K$时有$r_1\sim K$、$\omega_2\sim m$，而$\omega_2-r_2$仍为$m$的量级，所以$|D_{\rm pair}|$至少为$Km$的量级。与此同时，精确测度中的$\sin^3(x\theta)$和Jacobian分别给出$x^3\theta^3$和$Kx$，足以将这一窄区间的贡献压低到
...
具体参数下的比较也显示了这一近似与精确几何之间的差别。取$K=1$、$\delta=0.03$、$m=3\times10^{-5}$，使用精确正弦几何和精确能量积分，提出公共$\alpha$后得到$0.8089721288$；用式\eqref{eq:c26-exact-angular-integral}再对$x$积分则为$0.8089671307$，差为$4.9981\times10^{-6}$。保持$m/(K\delta)$不变而将$\delta$增至0.1，差增为$5.5543\times10^{-5}$，与这组参数下的小角平方修正相符。

## 入射配置与四条腿的求和 {#c26-initial-states}

两个出射硬方向彼此分离，因此另一条出射腿也给出同样的式\eqref{eq:c26-single-leg-result}。若还要为两条入射腿各加上一份相同贡献，就须指定初态制备：末态探测器怎样合并读数，与束流中包含多少额外入射粒子，是事件概率中的两种不同权重。
...
\tag{C26.32}\label{eq:c26-initial-density}
$$
...
(2\pi)^5\,2\bar\omega\,
\delta^5(\mathbf k_1+\mathbf k_2-\mathbf k).
\tag{C26.33}\label{eq:c26-symmetric-cluster-weight}
$$

在有限体积和有限硬动量区间内，这给出一种可归一化的正混合制备权重。与同一制备下的Born参考率作比时，共同归一化因子一起提出。若实际两粒子簇的权重随$x$变化，则应将它乘回积分，所得结果也会相应改变。

对于实标量理论，时间反演使配对过程的平方矩阵元相等。两条共线入腿先融合，再进入硬过程图块，局部顶点与内部虚度仍给出$g^2/(k_{\rm pair}^2+m^2)^2$。因此，配以式\eqref{eq:c26-symmetric-cluster-weight}的相同局部权重后，每条入腿的积分便等于一条出腿的积分。四条硬腿在相对$O(\alpha)$各贡献一份，合起来得到
...
\right].
\tag{C26.34}\label{eq:c26-four-leg-result}
$$

若只指定末态分辨率，而初态仍固定为两个粒子，同一计算只包含两条出腿的未分辨分裂贡献。

## 与虚圈合并后还剩什么 {#c26-real-virtual}

有了真实分裂的概率修正，现在将它与原硬过程的虚圈修正合并。对式\eqref{eq:c26-hard-log}取模方时，共同的实质量对数在振幅及其共轭中各出现一次，因而
...
\tag{C26.35}\label{eq:c26-virtual-square}
$$
...
=L+\ln\delta^2-\ln4+c.
\tag{C26.36}\label{eq:c26-cm-log}
$$

有限组合$-\ln4+c$可并入式\eqref{eq:c26-virtual-square}的其他有限项。按四腿规定将真实与虚圈贡献相加，并只保留同一弱耦合阶，两个大对数分别为
...
\right]+O(\alpha^2).
\end{aligned}
\tag{C26.37}\label{eq:c26-residual-logs}
$$
...
\tag{C26.38}\label{eq:c26-perturbative-log-domain}
$$

当大对数使高阶项不再比领先项小，就需要将更高阶贡献纳入处理。另一方面，式\eqref{eq:c26-residual-logs}中仍有$\tfrac32\ln(s/m^2)$，说明在当前耦合定义下，单靠修正事件计数尚不能得到有限的无质量极限。这个残项将引向下一章：继续检查耦合常数的重整化条件。
