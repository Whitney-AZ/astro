# 第73章 非阿贝尔规范理论中的贝塔函数 {#c73}

上一章已经给出了计算圈图所需的规则。现在要回答的问题是：杨–米尔斯耦合常数
怎样随能标改变？在电动力学中，电子场与顶角的重整化因子相消，电荷的运行由
光子自能决定。非阿贝尔理论多出规范场的自相互作用，这个相消关系随之改变。
先求夸克场和夸克—胶子顶角的反项，再计算胶子自能，
最后把三个结果组合成同一个裸耦合。

计算取Feynman规范和$\overline{\mathrm{MS}}$方案，沿用
[本章约定](../conventions.md#conventions-s73)中的$d=4-\epsilon$。
以下$C_R=C(R)$、$C_A=T(A)$，生成元归一由第70章固定；
先考虑一个简单规范群因子，以及$n_F$个属于同一不可约表示$R$的Dirac味。
可约物质按不可约块分开计算，最后相加。圈积分简记为
$\int_\ell=\int d^d\ell/(2\pi)^d$，鬼场记为$c,\bar c$。

## 从各个顶角回到同一个耦合 {#c73-bare}

把上一章的每一类动能和相互作用分别乘以重整化因子。
...
\end{aligned}
\tag{C73.01}\label{eq:c73-renormalized-density}
$$
...
\end{gathered}
\tag{C73.02}\label{eq:c73-bare-fields}
$$

最后一个选择使裸规范固定项
$-(\partial\cdot A_0)^2/(2\xi_0)$正好成为
$-(\partial\cdot A)^2/(2\xi)$，所以它在式\eqref{eq:c73-renormalized-density}
...
\end{aligned}
\tag{C73.03}\label{eq:c73-vertex-bare-matching}
$$
...
\end{aligned}
\tag{C73.04}\label{eq:c73-common-bare-coupling}
$$

这段代数说明同一个裸耦合要求哪些关系。量子反项能否同时满足它们，
还需要非阿贝尔的Slavnov–Taylor恒等式及无规范异常的条件，其根据见
[第74章](74.md#c74-st)。先以这一量子关系为条件，任选一个顶角就能
确定$g_0$。夸克顶角可复用第62章的大部分积分，因而这里只需求$Z_1,Z_2,Z_3$。

## 夸克自能的颜色因子 {#c73-quark}

图\ref{fig:c73-quark-self-energy}左边只有一条内部胶子线。
它把两个生成元的颜色指标相接，留下
$\sum_a(T^aT^a)_{ij}=C_R\delta_{ij}$。其余的动量与gamma矩阵运算
同[电子自能](62.md#c62-electron-loop)完全一样。

\begin{figure}[htbp]
\centering
\input{notes/figures/s73_quark_self_energy.tex}
\renewcommand{\thefigure}{C73a}
\caption{夸克自能与局部反项。
实线箭头从$j$到$i$，两顶点间带$p+\ell$；
上方胶子从右向左带$\ell$。叉号包括动能和质量反项。}
\label{fig:c73-quark-self-energy}
\end{figure}

具体地，把式\eqref{eq:c62-electron-parameters}中的$e^2$换成$g^2C_R$，并取零胶子质量。
平移$q=\ell+xp$后，奇的$\not q$项积分为零，于是
...
\end{aligned}
\tag{C73.05}\label{eq:c73-quark-parameter-integral}
$$

为提取UV极点，可以在欧氏离壳动量或$m>0$下进行。第62章的主积分极部为
...
\end{aligned}
\tag{C73.06}\label{eq:c73-quark-counterterms}
$$

第一行的两个独立矩阵结构分别确定动能和质量反项。
质量项在求贝塔函数时不用，却可直接用于[习题73.3](../exercises/73.md#ex73-3)。
这里保留的是$\overline{\mathrm{MS}}$极部。第62章为满足壳上质量和留数条件
加入的有限项，不能一同带到本章。
...
图\ref{fig:c73-quark-vertices}左边是已经算过的电动力学顶角图。
...
\end{aligned}
\tag{C73.07}\label{eq:c73-abelian-color-chain}
$$

第二行用$f$在$b,c$下反对称，使反对易部分消失；最后一行将
$f^{bcd}=f^{dbc}$后，用第70章的伴随迹
$f^{abc}f^{dbc}=C_A\delta^{ad}$。这将有序的三个生成元约化为一个生成元。
把[第62章已求出的顶角极部](62.md#c62-vertex-uv)乘上这个颜色因子，
第一幅图给
...
\tag{C73.08}\label{eq:c73-abelian-vertex-pole}
$$

第62章计算中，两个费米传播子的分子产生
$(d-2)^2q^2\gamma^\mu/d$，带$q^2$的三分母积分的极部为
$i/(8\pi^2\epsilon)$；Feynman参数的总权重为1。
本次只改变有序颜色链和耦合，故没有新的自旋迹或味数因子。
费米线仍是开链，也没有闭圈负号。

\begin{figure}[htbp]
\centering
\input{notes/figures/s73_quark_vertices.tex}
\renewcommand{\thefigure}{C73b}
\caption{夸克—胶子顶角的两种一圈修正。
外胶子为$(a,\mu)$。左图的内部胶子颜色为$b$；
右图两胶子的颜色为$b,c$，它们在下方三胶子顶角会合。
右图按零外动量标记，内部费米线带$\ell$，三胶子端的两个内部出动量为$\ell,-\ell$。}
\label{fig:c73-quark-vertices}
\end{figure}

右图含有三胶子顶角，是本章第一个新的积分。它的UV发散度为零，
局部发散项因而不能含外动量。取$m>0$并令外动量为零，就能直接取出该系数；
保留$m$也使约分后的积分在红外可积。无质量理论可先保留非零离壳动量
或一个辅助红外尺度，UV减除后再去掉它。
图中左、右端的内部Lorentz指标标为$\rho,\nu$；
...
\end{aligned}
\tag{C73.09}\label{eq:c73-nonabelian-vertex-integral}
$$
...
=-\frac i2 C_AT^a .
\tag{C73.10}\label{eq:c73-nonabelian-color}
$$
...
\end{aligned}
\tag{C73.11}\label{eq:c73-nonabelian-numerator}
$$

这就完成了分子的gamma缩并。因为现在只求简单UV极点，
...
\tag{C73.12}\label{eq:c73-cancelled-vertex-denominator}
$$
...
\end{aligned}
\tag{C73.13}\label{eq:c73-vertex-master-evaluation}
$$

第三行用了$\int_0^1x^{-\epsilon/2}dx=(1-\epsilon/2)^{-1}$。
Wick旋转、径向Gamma积分及$\mu$的换算均与式\eqref{eq:c62-master-integral}相同。
所以$I_m$的极点为$i/(8\pi^2\epsilon)$。式中虽列出$I_m$的有限项，
完整顶角的有限部分还会接收式\eqref{eq:c73-nonabelian-numerator}中
...
\end{aligned}
\tag{C73.14}\label{eq:c73-vertex-counterterm}
$$

多出的$C_A$正是规范场自相互作用的结果。
在电动力学中，Ward恒等式使$Z_1=Z_2$；这里规范固定及鬼作用量同时含普通导数
和协变导数，不能只从物质动能的形式照搬那个结论。
本章使用的是式\eqref{eq:c73-common-bare-coupling}的各比值相等，
它容许单独的$Z_1,Z_2$不同。下面还需求出其中的$Z_3$。

## 胶子自能：两个三胶子顶角的缩并 {#c73-gluon}

胶子二点函数包含胶子蝌蚪、胶子泡、鬼泡、夸克泡和局部反项，
如图\ref{fig:c73-gluon-self-energy}所示。先看蝌蚪：四胶子顶角不含导数，
...
\end{aligned}
\tag{C73.15}\label{eq:c73-scaleless-tadpole}
$$

前两行原来分别在$\operatorname{Re}d>2$和$\operatorname{Re}d<2$收敛；
延拓后的和不依赖$M$。这沿用第65章的无尺度积分约定。
后面的泡图含外动量$k$，不能按同样理由消去；先取$k^2>0$的欧氏区域。

\begin{figure}[htbp]
\centering
\input{notes/figures/s73_gluon_self_energy.tex}
\renewcommand{\thefigure}{C73c}
\caption{胶子二点函数的一圈图及反项。
上排为四胶子蝌蚪和胶子泡，下排为鬼泡、夸克泡及反项。
两条外线的动量$k$均向右，内部泡的上支带$\ell+k$、下支带$\ell$。
鬼泡的点线和夸克泡的实线均有连续箭头；胶子泡的两条无向内线可交换。}
\label{fig:c73-gluon-self-energy}
\end{figure}
...
-C_\sigma\delta^\mu_\rho
 &-C^\mu L^\nu&C^\nu B^\mu&C^2g^{\mu\nu}
\end{array}
\tag{C73.17}\label{eq:c73-nine-contractions}
$$
...
=-\left[(d-2)L^\mu L^\nu
 +(B^2+C^2)g^{\mu\nu}
 +B^\mu C^\nu+C^\mu B^\nu\right].
\tag{C73.18}\label{eq:c73-contracted-numerator}
$$
...
\end{aligned}
\tag{C73.19}\label{eq:c73-gluon-parameter-shift}
$$

平移的Jacobian为1。把最后一行代入式\eqref{eq:c73-contracted-numerator}，
奇的$q$项积分为零。$q^\mu q^\nu$的系数由
...
\end{aligned}
\tag{C73.20}\label{eq:c73-even-numerator}
$$
...
=-\frac d{d-2}D\,I_2(D).
\end{aligned}
\tag{C73.21}\label{eq:c73-radial-integrals}
$$

$J$的关系来自式\eqref{eq:c62-radial-identity}：先在固定$D>0$、$0<\operatorname{Re}d<2$
...
为了取简单极点，先在式\eqref{eq:c73-even-numerator}中用
$q^\mu q^\nu\to q^2g^{\mu\nu}/4$，并令$d=4$。三个系数分别算为
$-2-(16-6)/4=-9/2$、
...
&+(2+10x-10x^2)k^\mu k^\nu .
\end{aligned}
\tag{C73.22}\label{eq:c73-four-dimensional-numerator}
$$
...
=-(5-11x+11x^2)k^2g^{\mu\nu}
 +(2+10x-10x^2)k^\mu k^\nu ,
\tag{C73.23}\label{eq:c73-reduced-pole-numerator}
$$
...
\left[-\frac{19}{6}k^2g^{\mu\nu}
 +\frac{11}{3}k^\mu k^\nu\right].
\end{aligned}
\tag{C73.24}\label{eq:c73-gluon-bubble-pole}
$$
...
\int_\ell\frac{(\ell+k)^\mu\ell^\nu}
 {(\ell^2-i0)[(\ell+k)^2-i0]} .
\end{aligned}
\tag{C73.25}\label{eq:c73-ghost-bubble-integral}
$$
...
\frac{k^2g^{\mu\nu}}{d-2}+k^\mu k^\nu\right]I_2(D).
\end{aligned}
\tag{C73.26}\label{eq:c73-ghost-tensor-reduction}
$$
...
\frac53(k^2g^{\mu\nu}-k^\mu k^\nu).
\end{aligned}
\tag{C73.27}\label{eq:c73-gauge-ghost-pole}
$$

鬼场的闭圈负号和颜色次序在这里共同发挥作用。它补足了胶子泡的两个系数，
使发散部分具有规范动能的张量形式，因而可由一个$Z_3$吸收。

### 保留维数后的一圈横向性 {#c73-transverse}

还要考察整个一圈自能的横向性。只比较极点不足以确定有限部分是否横向，不过前面的计算已经保留了所需的$d$维分子，只须再作一次参数积分。
令$u=x(1-x)$，将式\eqref{eq:c73-even-numerator}中的
$q^\mu q^\nu$换成$g^{\mu\nu}q^2/d$，并用式\eqref{eq:c73-radial-integrals}
的精确关系。胶子泡的度规系数是
$-5+(8d-10)u/(d-2)$，$k^\mu k^\nu$系数是$6-d+(4d-6)u$。
在共同前因子$-g^2C_A/2$下，鬼泡须加两倍式\eqref{eq:c73-ghost-tensor-reduction}，
于是
...
\end{aligned}
\tag{C73.28}\label{eq:c73-full-dimensional-tensor}
$$

所有$x$依赖的权重为$I_2(uk^2)\propto u^{d/2-2}$。
记$a=d/2-2$，利用[附录E的Beta积分](../appendices/zz_loop_integrals.md#e-gamma-beta)，
其归一加权平均为
...
\end{aligned}
\tag{C73.29}\label{eq:c73-transverse-beta-integral}
$$

第二行只用了$\Gamma(z+1)=z\Gamma(z)$；代入上一式的最后一行，
常数项与$u$项正好抵消。这些参数积分可先在
$2<\operatorname{Re}d<4$、$k^2>0$下建立，再与先前完成的径向亚纯函数
一同延拓。于是$k_\mu(\Pi_G+\Pi_H)^{\mu\nu}=0$对完整的一圈函数成立，
并沿同一$i0$取到物理区域。这里的径向关系已经经过维数延拓，
并没有把原来二次发散的张量积分直接当成收敛的四维积分来平移。

## 夸克闭圈与胶子反项 {#c73-gluon-counterterm}

剩下的夸克泡可复用[第62章的完整横向分子](62.md#c62-photon-loop)。
旋量迹仍为4，但颜色迹独立给
$\operatorname{Tr}_R(T^aT^b)=T(R)\delta^{ab}$。
每一味可分别围成闭圈，因此还要对味求和。
将式\eqref{eq:c62-loop-transverse}中的$2x(1-x)$和四gamma迹的4一并保留，得到
...
(k^2g^{\mu\nu}-k^\mu k^\nu).
\end{aligned}
\tag{C73.30}\label{eq:c73-fermion-polarization}
$$
...
将它同式\eqref{eq:c73-gauge-ghost-pole}和式\eqref{eq:c73-fermion-polarization}相加，
得到圈图与反项之和：
...
\end{aligned}
\tag{C73.31}\label{eq:c73-gluon-counterterm}
$$
...
\frac{g^2}{8\pi^2\epsilon}+O(g^4).
\end{aligned}
\tag{C73.32}\label{eq:c73-three-renormalization-factors}
$$

这三个因子各自依赖场的归一和规范选择。规范耦合所要求的组合则是
$Z_1^2/(Z_2^2Z_3)$。同第66章一样，改用正的无量纲变量
...
+\sum_{n\ge1}\frac{G_n(\alpha)}{\epsilon^n}.
\end{aligned}
\tag{C73.33}\label{eq:c73-bare-alpha-log}
$$
...
\end{aligned}
\tag{C73.34}\label{eq:c73-simple-pole}
$$

第一行依次来自$2\ln Z_1,-2\ln Z_2,-\ln Z_3$。
开放夸克线的$C_R$项在这里相消；闭夸克圈的$T(R)$则留下来。
因而用哪一种夸克顶角定义耦合，并不会给规范部分引入一个额外的外腿Casimir。
...
对式\eqref{eq:c73-bare-alpha-log}求导，按极点次数整理：
...
\tag{C73.36}\label{eq:c73-alpha-beta}
$$
...
\end{aligned}
\tag{C73.39}\label{eq:c73-one-loop-running}
$$
...
复标量和多种物质表示的贡献，以及质量、场的反常维数，见
[本章习题](../exercises/73.md#ex73)。下一章将说明共同裸耦合关系所需的量子条件，讨论规范固定后的BRST对称性。

[^s73-index-print]:
    内部Lorentz指标的升降也作用于相应的动量分量和度规。
    将两只顶角的内部指标分别统一置下、置上后，按通常的矩阵缩并逐项相乘；
    不能只改变某一个度规的指标位置而保持同项其余因子不变。
