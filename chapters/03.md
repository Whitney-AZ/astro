...
上一章确定了标量场随洛伦兹变换和平移变换的规律，现在要构造满足这些规律的动力学。我们从第1章已有的自由多粒子哈密顿量出发，先把能量改为相对论形式，再问：这样的粒子理论能否由一个局域场得到？为回答这个问题，转而研究经典实标量场的作用量和正则变量，再将它们量子化。最后得到的哈密顿量将重新给出正能自由粒子的谱，并说明为什么这种实标量场构造选择了玻色统计。

## 从已有的多粒子理论提出问题 {#c03-fourier}

先暂时只令 $\hbar=1$，保留 $c$。第1章的无相互作用哈密顿量是 $H=\int d^3x\,a^\dagger(-\nabla^2/2m)a$。自由粒子的动量不随时间改变，因此在动量表象中，哈密顿量应当最为简单。定义如下傅里叶变换，并为这一归一化的动量算符保留波浪号：
...
\end{aligned}
\tag{C03.01}\label{eq:c03-fourier}
$$
...
\end{aligned}
\tag{C03.02}\label{eq:c03-free-nr}
$$
...
要把这些模式解释为产生和湮灭算符，还须变换它们的代数。由位置空间的CCR或CAR出发，对两种统计都有
...
\end{aligned}
\tag{C03.03}\label{eq:c03-fourier-bracket}
$$

同类算符的两个括号也为零，因为相应的位置空间括号本来就是零。定义真空满足 $\widetilde a(\mathbf p)|0\rangle=0$后，便可逐个加入动量模式来构造多粒子态。第1章已经求过偶双线性哈密顿量与产生算符的普通对易子；这里的一体核是动量对角函数 $E(\mathbf p)$，所以相应结果为
...
这些链给出了一组可选的广义共同本征基。若不同动量配置的总能量相同，它们的线性组合仍是能量本征态，却一般不再对应一条单独的确定动量产生链。能量的简并性因而不允许把任意能量本征态都解释为动量配置确定的态。
...
\widetilde a^\dagger(\mathbf p)\widetilde a(\mathbf p)
\tag{C03.04}\label{eq:c03-rel-start}
$$

这样就得到正能、可加的自由粒子谱。但正确的谱还没有显现出局域场怎样变换。为把这个粒子图像与上一章的协变性联系起来，下面从经典标量场重新构造同一动力学，并在量子化过程中考察统计选择的影响。

## 经典场、作用量与自然单位 {#c03-action}
...
\end{aligned}
\tag{C03.07}\label{eq:c03-variation}
$$

若取紧支撑变分，或要求边界上 $\delta\varphi=0$，表面项便消失。其余积分对任意测试变分都为零，因而得到式\eqref{eq:c03-kg}。这里将变分所得的方程整体乘了 $-1$，不改变解集。
...
\end{aligned}
\tag{C03.08}\label{eq:c03-legendre}
$$

哈密顿形式明确了一个场在固定时刻需要的正则资料：空间每一点都有场坐标和共轭动量，共轭动量本身是密度，哈密顿密度还须对空间积分才给出能量。下面反演模式系数时，正好要同时使用这两份资料。

## 实性如何联系两支频率 {#c03-modes}

自由场的方程对不同空间波矢彼此独立，因而适合用傅里叶变换求解。对空间作变换后，式\eqref{eq:c03-kg}化为常系数方程
$\ddot\varphi_{\mathbf k}+\omega_{\mathbf k}^2\varphi_{\mathbf k}=0$，其中 $\omega_{\mathbf k}=\sqrt{\mathbf k^2+m^2}>0$。先取 $m>0$，并选光滑且迅速衰减的初始数据，以便直接作傅里叶积分。若只要求场在空间无穷远有界，未必能保证普通傅里叶积分逐点收敛，较一般的展开须按温和分布理解。每个模式都是二阶时间方程，需要两份初始数据，因此一般解保留两支频率：
...
\tag{C03.09}\label{eq:c03-two-frequency}
$$
...
\tag{C03.10}\label{eq:c03-reality}
$$
...
\tag{C03.11}\label{eq:c03-real-modes}
$$

实性联系的是正、负频率两支，而 $A(\mathbf k)$与 $A(-\mathbf k)$仍可独立选择，并不要求 $A(-\mathbf k)=A^*(\mathbf k)$。对一对非零的相反波矢，可以用余弦和正弦两份实坐标计数，也可以用两份行波振子计数。因此式\eqref{eq:c03-real-modes}仍须对全部空间波矢求和，删去一半波矢就会丢掉一半自由度。

## 为什么不变测度含有 $2\omega$ {#c03-measure}

展开中的四动量满足 $k^2=-m^2$，称为在质量壳上（on shell）。系数只需用 $k^0>0$壳上的点编号，负频率部分已由共轭项给出。为了使展开便于作洛伦兹变换，现在选择归一化函数，使三维测度成为质量壳上的不变测度。可从四维形式 $d^4k\,\delta(k^2+m^2)\theta(k^0)$出发：$d^4k$和壳约束都不变，正时洛伦兹变换又保持正能支，所以整个测度不变。含时间反演的变换会交换能量分支，不在这一论证的范围内。
...
\tag{C03.12}\label{eq:c03-delta-jacobian}
$$
...
\end{aligned}
\tag{C03.13}\label{eq:c03-measure}
$$
...
\left[a(\mathbf k)e^{ikx}+a^*(\mathbf k)e^{-ikx}\right].
\tag{C03.14}\label{eq:c03-field-expansion}
$$

把所有模式写在同一个不变测度下以后，参考系改变时只需重新标记质量壳上的点。也可以直接用三维Jacobian看出这一点。沿第一轴取速度参数 $\beta$，有
$k'_1=\gamma(k_1-\beta\omega)$、$k'_{2,3}=k_{2,3}$、
$\omega'=\gamma(\omega-\beta k_1)$。由于 $\partial\omega/\partial k_1=k_1/\omega$，三维换元矩阵的后两行是单位行，只须求第一行的导数。这里用了相对运动参考系常见的负非对角元写法；相对于第2章正参数 $\eta$的主动推动，参数关系为 $\beta=-\tanh\eta$。在这一约定下，

$$
\det\frac{\partial\mathbf k'}{\partial\mathbf k}
=\gamma\left(1-\frac{\beta k_1}{\omega}\right)
=\frac{\omega'}{\omega}>0.
\tag{C03.15}\label{eq:c03-boost-jacobian}
$$

因此 $d^3k'/\omega'=d^3k/\omega$，与四维壳约束的计算一致。在 $m=0$的情形，连续三维测度在 $\mathbf k=0$附近仍局部可积，但该点不再是上述一维delta公式的简单零点。有限盒中恰好零波数的模式尤其不能除以 $\omega=0$，须作为自由坐标单独处理，所以本章的振子构造先取 $m>0$。

## 从场的初始数据反演模式 {#c03-inversion}

展开式给出了从模式到场的映射；要将场的正则关系转成模式的量子代数，还需要把它反解。式\eqref{eq:c03-field-expansion}同时含正负两项，因此只对 $\varphi$作空间傅里叶变换还不能分离出 $a$。利用同一时刻的场和速度，定义两份投影
...
\end{aligned}
\tag{C03.16}\label{eq:c03-two-projections}
$$
...
a^*(\mathbf k)&=\int d^3x\,e^{ikx}(\omega\varphi-i\Pi).
\end{aligned}
\tag{C03.17}\label{eq:c03-inverse}
$$
...
\tag{C03.18}\label{eq:c03-mode-constant}
$$

最后一步对空间作两次分部积分，边界项按本章的衰减假设消失。这说明反演系数确实与所选时刻无关。量子化后，反演仍带有显式时间核，其不随时间改变的含义不变；通常的振子海森堡算符则是 $a_H(t)=e^{-i\omega t}a$，时间因子已被放在算符中。两种写法只是把同一模式的时间依赖放在不同位置。

## 正则量子化与模式对易关系 {#c03-ccr}
...
\end{gathered}
\tag{C03.19}\label{eq:c03-canonical}
$$

这把量子力学的 $[q_j,p_l]=i\delta_{jl}$推广到连续的坐标标签。场是算符值分布，关系式应先以测试函数涂抹；同点二次能量可先用有限盒和动量截断定义，减去指定的真空常数后再去掉截断。自由场中有限粒子数的光滑波包给出了这些计算的自然公共域。
...
\end{aligned}
\tag{C03.20}\label{eq:c03-mode-ccr}
$$
...
\tag{C03.21}\label{eq:c03-aa-zero}
$$

在delta的支撑上 $\mathbf q=-\mathbf k$，色散又是偶函数，因而整个系数为零。再取厄米共轭，得 $[a^\dagger,a^\dagger]=0$，至此求出了全部模式对易关系，也完成了[习题3.1](#ex03-1)所需的计算。
...
=i\delta^3(\mathbf x-\mathbf y).
\tag{C03.22}\label{eq:c03-ccr-back}
$$

两个频率部分各贡献一半。对于 $\varphi\varphi$和 $\Pi\Pi$的对易子，得到的则是两个指数之差，所乘的权重分别为偶函数 $1/(2\omega)$和 $\omega/2$；作 $\mathbf k\mapsto-\mathbf k$换元，两项便相消。这说明在前述函数与算符域条件下，场和模式的两种CCR描述彼此相容。场中的负频率指数已乘着产生算符，是否产生正能量应由哈密顿量的作用判断，而不能只凭指数的正负号判断。

## 哈密顿量中四类模式项的去向 {#c03-hamiltonian}

现在用模式重写能量，看看场量子是否具有本章开头所需的相对论粒子谱。经典乘积 $a^*a$量子化后会涉及排序，不同排序可相差常数。因此先保留场哈密顿量中所有因子的次序，待空间积分完成以后，再用对易关系整理成 $a^\dagger a$的形式。先写出场及其时间、空间导数：
...
\end{aligned}
\tag{C03.23}\label{eq:c03-derivative-modes}
$$

把它们逐项代入式\eqref{eq:c03-legendre}，即可按频率组合整理能量。$\Pi^2$的同频率系数是 $(-i\omega)(-i\omega')=-\omega\omega'$，梯度平方贡献 $-\mathbf k\cdot\mathbf q$，质量项贡献 $m^2$；相反频率时，前两项都变为正号。为合并三种贡献，定义
...
\end{aligned}
\tag{C03.24}\label{eq:c03-h-four-terms}
$$
...
\tag{C03.25}\label{eq:c03-h-symmetric}
$$
...
\end{aligned}
\tag{C03.26}\label{eq:c03-box}
$$

量子实标量场由此分解成一组谐振子，每个空间波矢对应一个模式。正、负波矢都在求和内，两支频率共同保证场的厄米性，并不把空间模式数减半。从有限模式的 $\varphi,\Pi,\nabla\varphi$直接计算能量，也得到相同的模式计数。

## 真空能：先算出发散，再规定能量零点 {#c03-vacuum}

每个谐振子都有零点能，把无穷多个模式相加便会产生真空能的问题。回到连续记号，用模式CCR将式\eqref{eq:c03-h-symmetric}中的 $aa^\dagger$移序。有限周期盒说明，其中出现的 $(2\pi)^3\delta^3(0)$表示 $V$的极限记号，而不是delta函数的普通点值。移序后得到
...
\sqrt{\mathbf k^2+m^2}.
\end{aligned}
\tag{C03.27}\label{eq:c03-vacuum-density}
$$
...
\end{aligned}
\tag{C03.28}\label{eq:c03-vacuum-integral}
$$
...
\right].
\tag{C03.29}\label{eq:c03-vacuum-exact}
$$
...
\tag{C03.30}\label{eq:c03-vacuum-asymptotic}
$$

在 $\Lambda\gg m$时，只保留首项的相对误差从 $m^2/\Lambda^2$阶开始。若 $m=0$，径向积分直接成为 $k^3$的积分，首项才是该截断下的精确结果；在 $\Lambda\lesssim m$时，则应使用完整闭式而非大截断近似。

现在可以决定保留在经典拉格朗日量中的常数。对于当前自由、平直时空、无引力耦合的模型，选择
$\Omega_0(\Lambda)=\mathcal E_0(\Lambda)$，就使 $H|0\rangle=0$。在固定背景上，所有态同时加同一能量常数只改变共同的时间相位，能差与跃迁概率不变，因此可以把真空选作能量零点。这里的三动量截断依赖于参考系；减去真空常数后，才能在自由Fock空间的适当域上取去截断极限。加入相互作用以后出现的其他发散还须分别处理。

## 粒子质量、态归一化与场的量纲 {#c03-particles}

减去真空常数后，哈密顿量只剩模式占据数项。要与本章开头的粒子哈密顿量直接比较，只须改回最初的模式归一化。令

$$
a(\mathbf k)=\sqrt{(2\pi)^3\,2\omega_{\mathbf k}}\,
\widetilde a(\mathbf k).
\tag{C03.31}\label{eq:c03-normalization-conversion}
$$

将它代入式\eqref{eq:c03-mode-ccr}，两个平方根在delta支撑上合并成 $(2\pi)^3 2\omega$，所以 $\widetilde a$满足式\eqref{eq:c03-fourier-bracket}的玻色CCR。代入 $H$时，同一因子消去 $d\widetilde k$的分母，得到
$H=\int d^3k\,\omega_{\mathbf k}\widetilde a^\dagger\widetilde a$。这正是式\eqref{eq:c03-rel-start}取自然单位后的形式：经典实标量场的量子化重新给出了自由相对论玻色子的能谱。
...
E_{\rm rest}=\hbar c m.
\tag{C03.32}\label{eq:c03-mass-restoration}
$$
...
\tag{C03.33}\label{eq:c03-state-norm}
$$

因此，对波包 $|f\rangle=\int d\widetilde k\,f(\mathbf k)|\mathbf k\rangle$求内积时，先用delta积掉一个动量，便消去两份测度中的一份，得到范数平方 $\int d\widetilde k\,|f|^2$。多粒子能量的可加性，以及这些态怎样作洛伦兹变换，见[本章习题详解](#ex03)的推导。

最后整理这些对象的量纲，以便随后加入相互作用。在四维自然单位中，$S$无量纲、$[\partial]=1$，动能项因而要求 $[\varphi]=1$，继而给出 $[\Pi]=2$、$[\mathcal L]=[\mathcal H]=[\Omega_0]=4$。不变测度有质量维数2，所以展开中的 $[a]=-1$；普通动量测度的维数为3，式\eqref{eq:c03-fourier-bracket}便给出 $[\widetilde a]=-3/2$，与式\eqref{eq:c03-normalization-conversion}相符。第1章的场 $a(\mathbf x)$只包含湮灭算符，这里的厄米场 $\varphi(x)$同时包含产生和湮灭部分。这两种场的定义和量纲都不同。

## 反对易尝试的两个不同问题 {#c03-car}

本章开头的自由多粒子谱允许两种统计，局域实标量场的正则量子化却给出了玻色子。为了理解差别，考察把对易关系换成反对易关系会发生什么。先从等时场关系着手：若将式\eqref{eq:c03-canonical}中的括号全部替换，对实测试函数涂抹后的厄米场 $\Phi_f$便有 $\{\Phi_f,\Phi_f\}=2\Phi_f^2=0$。在正定内积空间的适当公共域上，这意味着
$\|\Phi_f\psi\|^2=\langle\psi|\Phi_f^2|\psi\rangle=0$，所以场只能为零。两个厄米算符的反对易子本身又是厄米的，也不能等于非零纯虚数 $i\delta$。因此，直接替换等时场括号已经无法构造所需的非平凡实场。

模式反演也会显示同一困难。即使暂时只作形式代数，场反对易子换序时不变号，所得 $\{a,a^\dagger\}$的系数就成为 $\omega-\omega'$，在 $\delta^3(\mathbf k-\mathbf q)$上等于零，无法给出需要的 $2\omega$。因此，等时场的反对易关系与模式的反对易关系不能仍按这一反演互相推出。
...
=(\mathcal E_0-\Omega_0)V.
\tag{C03.34}\label{eq:c03-car-failure}
$$

哈密顿量只剩常数，因而 $[H,a^\dagger]=0$，不能给出所需的升能关系 $[H,a^\dagger]=\omega a^\dagger$。零点能常数也可吸收入 $\Omega_0$，但这不改变占据数项消失的结论。若另选 $H=\int d\widetilde k\,\omega a^\dagger a$，费米Fock空间仍可具有正能谱，但这已不是从本章局域实标量作用量得到的哈密顿量。下一章将进一步用局域性考察这种选择，从而说明自旋零场与统计的关系。

局域作用量也为相互作用提供了自然的引入办法。例如可以加入 $\varphi^3$或 $\varphi^4$：同一点标量的乘积仍是标量，所以这样的局域相互作用保持作用量的形式洛伦兹不变性。若将 $-\lambda_n\varphi^n/n!$加入 $\mathcal L$，四维耦合的质量维数为 $[\lambda_n]=4-n$。势能有无下界还取决于耦合符号及其他项，量子理论中的同点乘积也须定义；后续各章将围绕这些相互作用建立计算方法。
